...
def extract(pages):
    ...
    def blue_field_name(t: str) -> str:
        tl = t.lower()
        if tl.startswith("staff initials"):
            return "Staff Initials"
        if tl.startswith("comment"):
            return "Comment"
        return ""
    ...
    def looks_like_activity_title(t: str) -> bool:
        ...
        if "?" in t:
            return False
        tl = t.lower().strip()
        for q in ("does ", "did ", "do ", "has ", "have ", "was ", "were ", "what ", "when ", "where ", "who ", "why ", "how "):
            if tl.startswith(q):
                return False
        return True
    ...
    for page_index, lines in pages:
        ...
        while i < len(lines):
            ...
            # Always capture explicit blue input fields (these were missing in audits).
            if is_blue_heading_line(ln, t):
                bf = blue_field_name(t)
                if bf:
                    emit(out, page_seen, current_form or current_schedule or "", bf, page_index)
                # Blue headings also act as boundaries for label capture.
                if collecting:
