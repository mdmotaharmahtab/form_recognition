Looking at the error, there's a bug on line 72 where I'm trying to subtract a string from a float. The issue is here:

```python
next_line.y0 - field_parts[-1] > 10
```

`field_parts[-1]` is a string (text), not a y-coordinate. I need to track the y-coordinate separately.
...
The key fix: I now track `last_y` as a separate variable to store the y-coordinate of the last line added to `field_parts`, so the comparison `next_line.y0 - last_y > 10` works correctly with two floats.
