...
def extract(pages):
    ...
    def is_blue_label_colon(ln) -> bool:
        ...
        if not t:
            return False
        # Blue labels: often bold, but not always; keep size range fairly wide.
    ...
    def is_table_header_like(lines, ln) -> bool:
        # If multiple short bold black labels share the same y, it's likely a table header row.
        if ln.non_black:
            return False
    ...
    def is_candidate_question_line(lines, ln) -> bool:
        ...
        # Avoid activity/header strings that look like "X: ... #1"
    ...
    def is_bad_field_label(txt: str, form_name: str) -> bool:
        ...
        # Avoid activity-like headers ending in "#1" etc.
    ...
    def normalize_field_label(field: str, form_name: str) -> str:
        ...
        fn = strip_trailing_colon(norm_space(form_name))
        if fn and t == fn:
            # Try to salvage a real question by stripping common activity prefixes.
    ...
    def pick_best_question_anchor(lines, y_marker: float, y_lower_bound: Optional[float], x_marker: float) -> Optional[object]:
        cands = []
        ...
        for ln in lines:
            ...
            score = 0.0
            score += max(0.0, 240.0 - dy)  # prefer close
            score += min(len(t), 100) * 1.25
            score -= min(dx, 220.0) * 0.35  # prefer alignment with marker
            if ln.bold:
                score += 16.0
            if "?" in t:
                score += 28.0
    ...
    for page_idx0, lines in pages:
        ...
        # Track last good field within the same page only (avoid cross-page propagation).
        ...
        # Safety net: emit Comment if present even when Answer(s) matching fails.
