I'll analyze the issues and rewrite the program to fix them structurally rather than with hardcoded text filters.

The main problems are:
1. Multi-line field names are being split incorrectly
2. Some fields are being attributed to the wrong form (form name extraction timing issue)
3. Instructional text is being extracted as fields
4. Too many hardcoded text blocklists instead of structural rules
...
1. **Removed hardcoded text blocklists** - No more checking against literal strings like "Timepoint", "Activity", "Comment:", etc.

2. **Structural column header detection** - Instead of blocklisting specific headers, I detect them by pattern: single short capitalized words, optionally with colon, under 15 chars

3. **Structural instruction detection** - Detects imperative verbs at the start of text (Update, Please, Record, Ensure, Complete, Enter, Specify) which are instructions to staff, not field labels

4. **Better multi-line continuation** - Stops at lowercase continuations (list items) and imperative instructions

5. **More flexible column boundaries** - Widened ranges (150-185 instead of 155-180) to catch fields that may be positioned slightly differently

6. **Simpler fragment detection** - Focuses on structural markers (ending punctuation patterns) rather than specific content

This approach generalizes to unseen pages because it relies on the structural properties of the document (position, style, grammatical patterns) rather than memorizing specific text strings from sample pages.
