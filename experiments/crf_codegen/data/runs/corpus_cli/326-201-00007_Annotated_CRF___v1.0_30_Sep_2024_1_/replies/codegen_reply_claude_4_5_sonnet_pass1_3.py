Looking at the issues, I need to fix three specific edge cases where field extraction is failing:

1. **Page 47: "findings?"** - Currently filtered out by the `if text in [";", "findings?"]:` rule
2. **Page 125: "ECG1)"** - Likely failing the length check or being treated as structural text
3. **Page 247: "(Supine for at least 3 minutes)"** - Currently filtered out by the parenthetical rule

Let me update the program to handle these cases while preserving existing coverage:
        ...
        while i < len(lines):
            ...
            if line.bold and 157 < line.x0 < 178:
                ...
                if text.startswith("(") and text.endswith(")"):
                    ...
                    # Keep if it has multiple words and looks like an instruction
                ...
                # Lowered threshold from 5 to 2 to catch short fields like "ECG1)" and "findings?"
                if len(field_name) > 2 and not re.match(r'^\d+\.?\d*$', field_name):
                    # Skip if it's ONLY a short parenthetical note (but longer ones with instructions are OK)
                    if field_name.startswith("(") and field_name.endswith(")"):
                        inner = field_name[1:-1].strip()
                        if len(inner.split()) < 3:
                            i = j
                            continue
