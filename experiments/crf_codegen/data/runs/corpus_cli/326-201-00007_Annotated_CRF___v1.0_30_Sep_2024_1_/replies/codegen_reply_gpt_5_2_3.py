...
def _next_blue_label_stop_y(anchor_ln, block_lines, y1: float) -> float:
    y0 = float(anchor_ln.y0)
    stop_y = y1
    for ln in block_lines:
        yy = float(ln.y0)
        if yy <= y0 + 0.5 or yy > y1:
            continue
...
def _right_side_x_min(anchor_ln, activity_x: float, max_x: float) -> float:
    # Anchor-based (x1) is the primary gate; the small activity offset is a guard for noisy x1.
    return max(float(anchor_ln.x1) + 2.0, float(activity_x) + 0.06 * max_x)
...
def _right_side_evidence_count(
    ...
    y0 = float(anchor_ln.y0)
    y1 = y0 + y_window
    y_stop = _next_blue_label_stop_y(anchor_ln, block_lines, y1=y1)

    x_min = _right_side_x_min(anchor_ln, activity_x=activity_x, max_x=max_x)
...
def _is_left_margin_fieldish_blue_label(ln, max_x: float) -> bool:
    t = (ln.text or "").strip()
    if not t or _is_code_line_text(t):
        return False
    if not (ln.non_black and ln.bold and t.endswith(":")):
        return False
    if float(ln.x0) > 0.16 * max_x:
        return False
    # Avoid admitting pure punctuation-like labels.
...
def _extract_answer_entry_labels(block_lines, ans_hdr_ln, activity_x: float, header_cut: float, footer_cut: float, max_x: float) -> List[str]:
    ...
    # Right-of-label / right column: allow for templates where options start closer to the header.
    x_min = _right_side_x_min(ans_hdr_ln, activity_x=activity_x, max_x=max_x)
    x_max = 1e9
        ...
        for hdr_ln, y0, y1 in blocks:
            ...
            for ln in blue_labels:
                if ans_hdr is not None and ln is ans_hdr:
                    continue
                # Emit left-margin blue labels (e.g., Comment/Staff Initials) even when the entry area is blank in text extraction.
                if not _has_right_side_entry(ln, block_lines, activity_x=activity_x, max_x=max_x):
                    if not _is_left_margin_fieldish_blue_label(ln, max_x=max_x):
                        continue
            ...
            if ans_hdr is not None:
                ...
                x_min = float(activity_x) + 0.12 * max_x
                ev = 0
                for ln in block_lines:
