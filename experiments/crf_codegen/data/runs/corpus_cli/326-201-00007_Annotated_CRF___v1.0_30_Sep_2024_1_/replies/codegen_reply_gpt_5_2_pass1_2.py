...
def extract(pages: List[Tuple[int, list]]) -> List[Dict[str, object]]:
    ...
    def find_activity_rows(rows: List[Tuple[float, list]]) -> List[Tuple[float, str]]:
        ...
        for y, r in rows:
            ...
            ln_num_x = None
            for ln in r:
                # some pages place Line# a bit left of prior threshold
                if ln.x0 > 420 and is_line_number_text(ln.text):
                    ln_num_x = ln.x0
                    break
            if ln_num_x is None:
                continue
            ...
            for ln in r:
                ...
                if not getattr(ln, "bold", False):
                    continue
                if not (8.5 <= ln.size <= 12.0):
                    continue
                if not (110 <= ln.x0 <= 320):
                    continue
                if ln.x0 >= ln_num_x - 40:
                    continue
                # avoid placeholder-like furniture
    ...
    def is_left_label(ln) -> bool:
        ...
        if ln.x0 > 170:
            return False
        if not (8.0 <= ln.size <= 12.5):
            return False
        # avoid column headers at y~116
        if 90 <= ln.y0 <= 150:
            return False
    ...
    def is_answer_header(ln) -> bool:
        ...
        if not tx or is_machine_annot(tx):
            return False
        if not (getattr(ln, "non_black", False) and getattr(ln, "bold", False) and (8.5 <= ln.size <= 11.5)):
            return False
        if not (115 <= ln.x0 <= 330):
            return False
        if not tx.endswith(":"):
            return False
        if 90 <= ln.y0 <= 150:
    ...
    def looks_like_entry_mask(s: str) -> bool:
        ...
        if _re_date_token.search(s) or _re_time_token.search(s):
            return True
        # bare mask token line (sometimes emitted separately)
    ...
    def clean_underscores_preserve_masks(raw: str) -> Tuple[str, bool]:
        ...
        # common case: "________ (9999 char. max.)" -> keep only the parens, but mark constraint_only
        if "_" in s:
            first_us = s.find("_")
        ...
        # no underscores: keep as-is (may already include a mask)
        return s, False
    ...
    def is_question_like_text(s: str) -> bool:
        ...
        if _re_mask_bare.fullmatch(s):
            return False
        # question-ish if longer or ends with "?" or contains parentheses mask as part of label
        if len(s) >= 18:
            return True
        if s.endswith("?"):
            return True
    ...
    def is_question_line(ln) -> bool:
        ...
        if getattr(ln, "non_black", False):
            return False
        if not (8.0 <= ln.size <= 12.0):
            return False
        if not (95 <= ln.x0 <= 360):
            return False
        # skip obvious placeholders as standalone
        if _re_date_token.fullmatch(tx) or _re_time_token.fullmatch(tx):
            return False
        return True
    ...
    def add_field(page_1based: int, form: str, field: str, seen: set):
        ...
        # Avoid emitting a parenthesis-only constraint as a field by itself
        if field.startswith("(") and field.endswith(")") and _re_char_max_parens.fullmatch(field):
            return
        if field.startswith("(") and field.endswith(")") and _re_mask_inside_parens.fullmatch(field):
            return
    ...
    for page_idx0, lines in pages:
        ...
        body_lines = [ln for ln in lines if ln.y0 >= 110]
        ...
        for y0, y1 in block_ranges:
            ...
            if q_lines:
                ...
                # avoid emitting something that is clearly just the activity title when activity detection failed
                for q in joined:
                    if current_form and norm_space(q) == norm_space(current_form):
                        continue
                    add_field(page_idx0 + 1, form_name, q, page_seen)

            # --- extract left labels (Comment:, Staff Initials:, Barcode:, etc.) ---
            for ln in block_lines:
                if is_left_label(ln):
                    add_field(page_idx0 + 1, form_name, norm_space(ln.text), page_seen)

            # --- extract answer entry fields ---
            # If answer header exists, scan answer section; otherwise do a conservative scan in the block
            ...
            # group to rows to better combine wrapped fragments
            row_map = {}
            for ln in scan_lines:
                if is_machine_annot(ln.text):
                    continue
                # focus on answer content region; allow slightly left for some variants
                if ln.x0 < 150:
                    continue
                k = y_key(ln.y0)
            ...
            for y, tx in row_list:
                ...
                if last_option_y is not None and abs(y - last_option_y) <= 18.5:
                    # wrapped option continuation: ignore unless it's clearly an entry mask (underscores/masks)
                    if not looks_like_entry_mask(tx):
                        continue

                # label accumulation (multi-line labels before the entry line)
                is_entryish = looks_like_entry_mask(tx) and (has_letter(tx) or "_" in tx or _re_mask_inside_parens.search(tx) or _re_date_token.search(tx))
                if not is_entryish and has_letter(tx) and not tx.endswith(":") and not _re_only_punct.fullmatch(tx):
                    if not pending_label:
                        pending_label = tx
                        pending_label_y = y
                    else:
                        # join if close enough vertically
                ...
                if constraint_only:
                    ...
                    if pending_label and pending_label_y is not None and abs(y - pending_label_y) <= 28.0:
                        ...
                        # otherwise drop constraint-only artifacts like "(9999 char. max.)"
                        continue
                else:
                    # If it's mainly a mask/paren line, attach to pending label when appropriate
                    if pending_label and pending_label_y is not None and abs(y - pending_label_y) <= 28.0:
                ...
                # consume pending label once used for an entry
                pending_label = ""
                pending_label_y = None

        # keep schedule as fallback context
