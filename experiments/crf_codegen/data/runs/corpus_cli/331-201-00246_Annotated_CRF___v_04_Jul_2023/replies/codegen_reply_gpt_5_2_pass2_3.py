```python
import re
from bisect import bisect_left, bisect_right
from typing import List, Dict


_CODE_TOKEN_RE = re.compile(r"^\[[A-Za-z0-9][A-Za-z0-9_]{1,40}\]$")
_BRACKET_ANY_RE = re.compile(r"^\[.*\]$")
_TYPE_BRACKET_RE = re.compile(r"(?i)^\[\s*TYPE\s*:\s*.+\]$")
_DATEISH_RE = re.compile(
    r"(?i)\b(\d{1,2}[-/][A-Za-z]{3}[-/]\d{2,4}|\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{1,2}[-/]\d{1,2}[-/]\d{2,4})\b"
)
_TIMEISH_RE = re.compile(r"\b\d{1,2}:\d{2}\b")
_MOSTLY_PUNCT_RE = re.compile(r"^[\W_]+$")
_ENUM_PREFIX_RE = re.compile(r"^\(\s*\d{1,2}\s*\)\s+")


def extract(pages):
    out: List[Dict[str, object]] = []
    seen = set()
    current_form: str = ""

    for page_index0, lines in pages:
        if not lines:
            continue

        width = max((getattr(ln, "x1", 0.0) for ln in lines), default=0.0) or 1.0
        height = max((getattr(ln, "y1", 0.0) for ln in lines), default=0.0) or 1.0
        page1 = page_index0 + 1

        footer_y = height - max(55.0, 0.085 * height)

        def is_footer_line(ln) -> bool:
            if ln.y0 < footer_y:
                return False
            if getattr(ln, "size", 0.0) > 13.0:
                return False
            t = (ln.text or "").strip()
            if not t:
                return True
            if len(t) > 55:
                return False
            digitish = sum(ch.isdigit() for ch in t)
            return (digitish >= 1) and (ln.x0 > width * 0.20)

        core_lines = [ln for ln in lines if not is_footer_line(ln) and (ln.text or "").strip()]

        title = _detect_form_title(core_lines, width, height)
        if title:
            current_form = title

        code_lines = _find_code_lines(core_lines, width, height)
        type_ann_lines = _find_type_annotation_lines(core_lines, width, height)

        if _looks_like_navigation_table(core_lines, width, height, code_lines):
            # Structural index pages: no fields
            continue
        if _looks_like_revision_log_page(core_lines, width, height, code_lines):
            # Structural revision log pages: no fields
            continue

        fields: List[str] = []

        if _has_grid_evidence(core_lines, width, height, code_lines, type_ann_lines):
            fields.extend(_extract_table_header_fields(core_lines, width, height))

        fields.extend(_extract_fields_via_codes(core_lines, width, height, code_lines))
        fields.extend(_extract_fields_via_type_annotations(core_lines, width, height, type_ann_lines))
        fields.extend(_extract_question_fields(core_lines, width, height))
        fields.extend(_extract_fields_via_blanks(core_lines, width, height))

        fields = _post_filter_heading_like(fields, core_lines, width, height)
        fields = _post_filter_option_like(fields, core_lines, width, height)

        form_name = (current_form or "").strip()
        for f in _dedup_preserve_order(_clean_field_names(fields)):
            if not f:
                continue
            key = (form_name, f, page1)
            if key in seen:
                continue
            seen.add(key)
            out.append({"form_name": form_name, "field_name": f, "page": page1})

    return out


def _detect_form_title(lines, width: float, height: float) -> str:
    # Pass 1: prefer non-black title near top-left, medium-large.
    cands = []
    for ln in lines:
        t = (ln.text or "").strip()
        if not t or _is_bracket_line(t):
            continue
        if ln.y0 > max(220.0, 0.30 * height):
            continue
        if ln.x0 > min(240.0, width * 0.38):
            continue
        if not getattr(ln, "non_black", False):
            continue
        sz = getattr(ln, "size", 0.0)
        if sz < 13.0 or sz > 21.5:
            continue
        if _is_mostly_numeric(t) or _looks_like_filled_value(t):
            continue
        if len(t) < 3:
            continue
        cands.append(ln)

    if cands:
        cands.sort(key=lambda l: (l.y0, -getattr(l, "size", 0.0), l.x0))
        return (cands[0].text or "").strip()

    # Pass 2: fallback for black section titles used on some layouts.
    cands2 = []
    top_y = max(140.0, 0.26 * height)
    for ln in lines:
        t = (ln.text or "").strip()
        if not t or _is_bracket_line(t):
            continue
        if getattr(ln, "non_black", False):
            continue
        if ln.y0 > top_y:
            continue
        if ln.x0 > width * 0.32:
            continue
        sz = getattr(ln, "size", 0.0)
        if not (9.8 <= sz <= 14.0):
            continue
        if _is_mostly_numeric(t) or _looks_like_filled_value(t):
            continue
        if len(t) < 3 or len(t) > 55:
            continue
        # Avoid column headers that look like fields
        if t.endswith("?") or t.endswith(":"):
            continue
        cands2.append(ln)

    if not cands2:
        return ""
    cands2.sort(key=lambda l: (l.y0, -getattr(l, "bold", False), -getattr(l, "size", 0.0), l.x0))
    return (cands2[0].text or "").strip()


def _find_code_lines(lines, width: float, height: float):
    top_cut = max(35.0, 0.04 * height)
    bot_cut = height - max(75.0, 0.10 * height)
    codes = []
    for ln in lines:
        t = (ln.text or "").strip()
        if not t:
            continue
        if not getattr(ln, "non_black", False):
            continue
        if not _CODE_TOKEN_RE.match(t):
            continue
        if ln.y0 < top_cut or ln.y0 > bot_cut:
            continue
        codes.append(ln)
    return codes


def _find_type_annotation_lines(lines, width: float, height: float):
    # Red annotations like "[TYPE: ...]" or red scale/anchor hints near top bands.
    top_cut = max(30.0, 0.035 * height)
    bot_cut = height - max(65.0, 0.11 * height)
    anns = []
    for ln in lines:
        t = (ln.text or "").strip()
        if not t:
            continue
        if not getattr(ln, "non_black", False):
            continue
        if ln.y0 < top_cut or ln.y0 > bot_cut:
            continue
        sz = getattr(ln, "size", 0.0)
        if sz > 11.5:
            continue
        if _TYPE_BRACKET_RE.match(t):
            anns.append(ln)
            continue
        # Some layouts show a red anchor line (e.g., "(0) ...") instead of a TYPE bracket.
        if t.startswith("(") and any(ch.isdigit() for ch in t[:6]) and len(t) <= 60:
            anns.append(ln)
            continue
        if "values:" in t.lower():
            anns.append(ln)
            continue
    return anns


def _looks_like_navigation_table(lines, width: float, height: float, code_lines) -> bool:
    if code_lines:
        return False

    header_band = [
        ln
        for ln in lines
        if (110.0 <= ln.y0 <= 155.0)
        and (8.0 <= getattr(ln, "size", 0.0) <= 10.5)
        and (not getattr(ln, "non_black", False))
        and getattr(ln, "bold", False)
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
        and not _is_mostly_numeric((ln.text or ""))
    ]
    if len(header_band) < 5:
        return False

    xs = sorted(ln.x0 for ln in header_band)
    spread = xs[-1] - xs[0]
    if spread < width * 0.55:
        return False

    blue_rows = [
        ln
        for ln in lines
        if getattr(ln, "non_black", False)
        and (8.0 <= getattr(ln, "size", 0.0) <= 10.5)
        and (ln.y0 >= 155.0)
        and (ln.y0 <= height - max(120.0, 0.15 * height))
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
    ]
    if len(blue_rows) < 10:
        return False

    x_blue = [ln.x0 for ln in blue_rows]
    if not x_blue:
        return False
    if (max(x_blue) - min(x_blue)) >= width * 0.20:
        return False

    numeric_col = [
        ln
        for ln in lines
        if (8.0 <= getattr(ln, "size", 0.0) <= 10.5)
        and (not getattr(ln, "non_black", False))
        and (ln.y0 >= 155.0)
        and (ln.y0 <= height - max(120.0, 0.15 * height))
        and _is_mostly_numeric((ln.text or "").strip())
        and (width * 0.20 <= ln.x0 <= width * 0.45)
    ]
    return len(numeric_col) >= 6


def _looks_like_revision_log_page(lines, width: float, height: float, code_lines) -> bool:
    if code_lines:
        return False

    title = _detect_form_title(lines, width, height)
    if not title:
        return False

    small_black = [
        ln
        for ln in lines
        if (not getattr(ln, "non_black", False))
        and (getattr(ln, "size", 0.0) <= 10.5)
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
        and (ln.y0 >= 160.0)
        and (ln.y0 <= height - max(120.0, 0.15 * height))
    ]
    if len(small_black) < 35:
        return False

    band = [ln for ln in small_black if 170.0 <= ln.y0 <= 270.0 and len((ln.text or "").strip()) <= 30]
    if len(band) < 4:
        return False

    best_cluster = []
    for ln in band:
        cluster = [x for x in band if abs(x.y0 - ln.y0) <= 6.0]
        if len(cluster) > len(best_cluster):
            best_cluster = cluster

    if len(best_cluster) < 4:
        return False

    xs = sorted(ln.x0 for ln in best_cluster)
    if (xs[-1] - xs[0]) < width * 0.45:
        return False

    dateish = 0
    for ln in small_black:
        t = (ln.text or "").strip()
        if _DATEISH_RE.search(t) or _TIMEISH_RE.search(t):
            dateish += 1
    if dateish < 3:
        return False

    return True


def _has_grid_evidence(lines, width: float, height: float, code_lines, type_ann_lines) -> bool:
    if len(code_lines) >= 1:
        return True
    if len(type_ann_lines) >= 1:
        return True

    for ln in lines:
        t = (ln.text or "").strip()
        if not t or _is_bracket_line(t):
            continue
        if "____" in t or "___" in t:
            return True
        if _contains_checkbox_glyph(t):
            return True
    return False


def _extract_table_header_fields(lines, width: float, height: float) -> List[str]:
    band = []
    for ln in lines:
        if ln.y0 < 90.0 or ln.y0 > 175.0:
            continue
        t = (ln.text or "").strip()
        if not t or _is_bracket_line(t):
            continue
        if getattr(ln, "non_black", False):
            continue
        sz = getattr(ln, "size", 0.0)
        if not (8.5 <= sz <= 12.8):
            continue
        if len(t) == 1 and _is_bullet_char(t):
            continue
        if _is_mostly_numeric(t):
            continue
        if _looks_like_filled_value(t):
            continue
        band.append(ln)

    if len(band) < 2:
        return []

    band.sort(key=lambda l: (l.x0, l.y0))

    col_clusters = []
    for ln in band:
        placed = False
        for col in col_clusters:
            if abs(col[0].x0 - ln.x0) <= 18.0:
                col.append(ln)
                placed = True
                break
        if not placed:
            col_clusters.append([ln])

    if len(col_clusters) < 2:
        return []

    col_xs = sorted(c[0].x0 for c in col_clusters)
    if (col_xs[-1] - col_xs[0]) < width * 0.40:
        return []

    fields = []
    for col in col_clusters:
        col.sort(key=lambda l: l.y0)
        y0_min = col[0].y0
        kept = [ln for ln in col if (ln.y0 - y0_min) <= 28.0 and not _looks_like_filled_value((ln.text or "").strip())]
        txt = " ".join((ln.text or "").strip() for ln in kept if (ln.text or "").strip())
        txt = _normalize_spaces(txt)
        if txt and not _looks_like_filled_value(txt) and not _is_mostly_numeric(txt):
            fields.append(txt)

    return fields


def _extract_fields_via_codes(lines, width: float, height: float, code_lines) -> List[str]:
    if not code_lines:
        return []

    black = [
        ln
        for ln in lines
        if (not getattr(ln, "non_black", False))
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
        and not _is_mostly_numeric((ln.text or ""))
        and not _looks_like_filled_value((ln.text or "").strip())
    ]
    if not black:
        return []

    black.sort(key=lambda l: (l.y0, l.x0))
    black_y = [ln.y0 for ln in black]

    results: List[str] = []

    for code_ln in code_lines:
        y0 = code_ln.y0
        y_min = y0 - 170.0
        y_max = y0 + 95.0
        i0 = bisect_left(black_y, y_min)
        i1 = bisect_right(black_y, y_max)

        window = []
        for ln in black[i0:i1]:
            if _acceptable_label_geometry(code_ln, ln, width):
                window.append(ln)

        if not window:
            continue

        best = max(window, key=lambda ln: _label_score_near_code(code_ln, ln))
        if _label_score_near_code(code_ln, best) < 1.5:
            continue

        merged = _merge_wrapped_label(best, window)
        merged = _normalize_spaces(merged)
        if not merged:
            continue
        if _is_bracket_line(merged) or _looks_like_filled_value(merged) or _is_mostly_numeric(merged):
            continue
        results.append(merged)

    return results


def _extract_fields_via_type_annotations(lines, width: float, height: float, type_ann_lines) -> List[str]:
    if not type_ann_lines:
        return []

    black = [
        ln
        for ln in lines
        if (not getattr(ln, "non_black", False))
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
        and not _is_mostly_numeric((ln.text or ""))
        and not _looks_like_filled_value((ln.text or "").strip())
    ]
    if not black:
        return []

    black.sort(key=lambda l: (l.y0, l.x0))
    black_y = [ln.y0 for ln in black]

    results: List[str] = []

    for ann in type_ann_lines:
        y0 = ann.y0
        # Prefer the header/label directly above the annotation (common in these layouts).
        y_min = y0 - 85.0
        y_max = y0 + 24.0
        i0 = bisect_left(black_y, y_min)
        i1 = bisect_right(black_y, y_max)

        window = []
        for ln in black[i0:i1]:
            if abs(ln.x0 - ann.x0) <= max(60.0, 0.10 * width):
                window.append(ln)

        if not window:
            continue

        best = max(window, key=lambda ln: _label_score_near_annotation(ann, ln))
        if _label_score_near_annotation(ann, best) < 1.2:
            continue

        merged = _merge_wrapped_label(best, window)
        merged = _normalize_spaces(merged)
        if not merged:
            continue
        if _is_bracket_line(merged) or _looks_like_filled_value(merged) or _is_mostly_numeric(merged):
            continue
        results.append(merged)

    return results


def _extract_question_fields(lines, width: float, height: float) -> List[str]:
    # Capture standalone question labels even when no codes are present.
    black = [
        ln
        for ln in lines
        if (not getattr(ln, "non_black", False))
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
        and not _looks_like_filled_value((ln.text or "").strip())
        and not _is_mostly_numeric((ln.text or ""))
    ]
    if not black:
        return []

    black.sort(key=lambda l: (l.y0, l.x0))

    results: List[str] = []
    used = set()

    for ln in black:
        if ln in used:
            continue
        t = _normalize_spaces((ln.text or "").strip())
        if not t:
            continue

        # Look for question ending; also allow wrapped questions where the first line doesn't end with '?'
        if not t.endswith("?") and len(t) < 25:
            continue

        # Merge down with same x/size to complete wrapped questions.
        run = [ln]
        used.add(ln)
        x_ref = ln.x0
        size_ref = getattr(ln, "size", 0.0)

        for nxt in black:
            if nxt in used:
                continue
            if abs(nxt.x0 - x_ref) > 18.0:
                continue
            if abs(getattr(nxt, "size", 0.0) - size_ref) > 2.0:
                continue
            dy = nxt.y0 - run[-1].y0
            if dy <= 0 or dy > 18.5:
                continue
            # Avoid pulling in value-ish content or bracket annotations
            nt = _normalize_spaces((nxt.text or "").strip())
            if not nt or _looks_like_filled_value(nt) or _is_bracket_line(nt):
                continue
            run.append(nxt)
            used.add(nxt)
            if nt.endswith("?"):
                break
            if sum(len((_normalize_spaces((x.text or "").strip()))) for x in run) > 240:
                break

        merged = _normalize_spaces(" ".join((_normalize_spaces((x.text or "").strip())) for x in run if (x.text or "").strip()))
        merged = _ENUM_PREFIX_RE.sub("", merged).strip()
        if merged and merged.endswith("?") and len(merged) >= 12:
            results.append(merged)

    return results


def _extract_fields_via_blanks(lines, width: float, height: float) -> List[str]:
    # Extract labels that appear immediately to the left/above underscore blanks or checkbox glyphs.
    black = [
        ln
        for ln in lines
        if (not getattr(ln, "non_black", False))
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
        and not _looks_like_filled_value((ln.text or "").strip())
        and not _is_mostly_numeric((ln.text or ""))
    ]
    if not black:
        return []

    candidates = []
    for ln in black:
        t = _normalize_spaces((ln.text or "").strip())
        if not t:
            continue
        if t.endswith("?"):
            continue
        if len(t) > 110:
            continue
        if _contains_checkbox_glyph(t) or "___" in t:
            continue
        if t.endswith(":"):
            candidates.append(ln)
            continue

        # If there's a nearby underscore-blank line to the right on same row, treat as label.
        for other in lines:
            ot = (other.text or "").strip()
            if not ot or _is_bracket_line(ot):
                continue
            if "___" not in ot and "____" not in ot:
                continue
            if abs(other.y0 - ln.y0) > 8.0:
                continue
            if other.x0 <= ln.x0:
                continue
            if (other.x0 - ln.x0) > max(460.0, 0.70 * width):
                continue
            candidates.append(ln)
            break

    if not candidates:
        return []

    # Merge wrapped labels for these candidates.
    results: List[str] = []
    for ln in candidates:
        window = [
            x
            for x in black
            if abs(x.x0 - ln.x0) <= 18.0
            and abs(getattr(x, "size", 0.0) - getattr(ln, "size", 0.0)) <= 2.5
            and abs(x.y0 - ln.y0) <= 28.0
        ]
        merged = _normalize_spaces(_merge_wrapped_label(ln, window))
        merged = merged.rstrip(":").strip()
        if merged and not _looks_like_filled_value(merged) and not _is_mostly_numeric(merged):
            results.append(merged)

    return results


def _acceptable_label_geometry(code_ln, label_ln, width: float) -> bool:
    cx0, cx1 = code_ln.x0, getattr(code_ln, "x1", code_ln.x0)
    lx0, lx1 = label_ln.x0, getattr(label_ln, "x1", label_ln.x0)

    left_gap = cx0 - lx1
    right_gap = lx0 - cx1
    gap = min(abs(left_gap), abs(right_gap), abs(lx0 - cx0))
    if gap > max(260.0, 0.38 * width):
        return False

    if (label_ln.x0 - code_ln.x0) > max(340.0, 0.50 * width):
        return False

    return True


def _label_score_near_code(code_ln, ln) -> float:
    t = (ln.text or "").strip()
    dy = ln.y0 - code_ln.y0
    dx = abs(ln.x0 - code_ln.x0)

    s = 0.0
    if dy <= 0:
        s += 10.0 - (abs(dy) * 0.10)
    else:
        s += 6.0 - (dy * 0.14)

    s += max(0.0, 6.0 - (dx * 0.02))

    if getattr(ln, "bold", False):
        s += 2.0
    if t.endswith("?"):
        s += 6.0
    if t.endswith(":"):
        s += 2.0

    L = len(t)
    if 3 <= L <= 90:
        s += 3.0
    elif L > 150:
        s -= 6.0

    if L > 120 and (t.count(".") + t.count(";") + t.count(",")) >= 2:
        s -= 4.0

    if _looks_like_filled_value(t):
        s -= 10.0

    return s


def _label_score_near_annotation(ann_ln, ln) -> float:
    t = (ln.text or "").strip()
    dy = ann_ln.y0 - ln.y0  # prefer label above annotation
    dx = abs(ln.x0 - ann_ln.x0)

    s = 0.0
    if dy >= 0:
        s += 10.0 - (dy * 0.18)
    else:
        s += 4.0 - (abs(dy) * 0.22)

    s += max(0.0, 6.0 - (dx * 0.03))

    if getattr(ln, "bold", False):
        s += 3.0
    if t.endswith("?"):
        s += 4.0
    if t.endswith(":"):
        s += 2.0

    L = len(t)
    if 3 <= L <= 70:
        s += 2.5
    elif L > 140:
        s -= 5.0

    if _looks_like_filled_value(t):
        s -= 10.0
    return s


def _merge_wrapped_label(seed, window) -> str:
    x_ref = seed.x0
    size_ref = getattr(seed, "size", 0.0)

    candidates = [
        ln
        for ln in window
        if abs(ln.x0 - x_ref) <= 16.0 and abs(getattr(ln, "size", 0.0) - size_ref) <= 2.5
    ]
    candidates.sort(key=lambda l: l.y0)

    seed_idx = None
    for i, ln in enumerate(candidates):
        if ln is seed:
            seed_idx = i
            break
    if seed_idx is None:
        return (seed.text or "").strip()

    run = [candidates[seed_idx]]

    i = seed_idx - 1
    while i >= 0:
        prev = candidates[i]
        cur = run[0]
        if (cur.y0 - prev.y0) > 24.0:
            break
        if _looks_like_filled_value((prev.text or "").strip()):
            break
        run.insert(0, prev)
        i -= 1

    i = seed_idx + 1
    while i < len(candidates):
        nxt = candidates[i]
        cur = run[-1]
        if (nxt.y0 - cur.y0) > 24.0:
            break
        if _looks_like_filled_value((nxt.text or "").strip()):
            break
        run.append(nxt)
        i += 1

    return " ".join((ln.text or "").strip() for ln in run if (ln.text or "").strip())


def _post_filter_heading_like(fields: List[str], lines, width: float, height: float) -> List[str]:
    if not fields:
        return fields

    by_text = {}
    for ln in lines:
        t = _normalize_spaces((ln.text or "").strip())
        if not t:
            continue
        if t not in by_text:
            by_text[t] = ln
        else:
            if getattr(ln, "size", 0.0) > getattr(by_text[t], "size", 0.0):
                by_text[t] = ln

    has_question = any(f.endswith("?") for f in fields)

    filtered = []
    for f in fields:
        ln = by_text.get(_normalize_spaces(f))
        if ln:
            sz = getattr(ln, "size", 0.0)
            bold = getattr(ln, "bold", False)
            t = f.strip()

            if bold and sz >= 12.0 and (not t.endswith("?")) and (not t.endswith(":")) and len(t) <= 40:
                continue

            if has_question and bold and sz >= 10.5 and ln.y0 <= min(0.38 * height, 320.0) and ln.x0 <= width * 0.45:
                if (not t.endswith("?")) and (not t.endswith(":")) and (len(t.split()) <= 5) and len(t) <= 45:
                    continue

            # Some layouts use small black section headers near the top-left; remove when real questions exist.
            if has_question and (not t.endswith("?")) and (not t.endswith(":")) and len(t) <= 38:
                if ln.y0 <= min(0.35 * height, 260.0) and ln.x0 <= width * 0.28:
                    if (bold or sz >= 10.4):
                        continue

        filtered.append(f)

    return filtered


def _post_filter_option_like(fields: List[str], lines, width: float, height: float) -> List[str]:
    if not fields:
        return fields

    # Map normalized text to a representative line object.
    by_text = {}
    for ln in lines:
        t = _normalize_spaces((ln.text or "").strip())
        if not t:
            continue
        if t not in by_text:
            by_text[t] = ln
        else:
            if getattr(ln, "size", 0.0) > getattr(by_text[t], "size", 0.0):
                by_text[t] = ln

    # Precompute black lines for neighborhood checks.
    black = [
        ln
        for ln in lines
        if (not getattr(ln, "non_black", False))
        and (ln.text or "").strip()
        and not _is_bracket_line((ln.text or ""))
    ]

    def has_parent_prompt(ln) -> bool:
        # Look for a longer prompt line above in the same column.
        x_ref = ln.x0
        y_ref = ln.y0
        sz_ref = getattr(ln, "size", 0.0)
        for up in black:
            if up.y0 >= y_ref:
                continue
            if (y_ref - up.y0) > 90.0:
                continue
            if abs(up.x0 - x_ref) > 22.0:
                continue
            ut = _normalize_spaces((up.text or "").strip())
            if not ut or _is_bracket_line(ut) or _looks_like_filled_value(ut) or _is_mostly_numeric(ut):
                continue
            if len(ut) >= 48 or ut.endswith("?") or ut.endswith(":"):
                if getattr(up, "size", 0.0) >= (sz_ref - 0.8):
                    return True
        return False

    def listlike_density(ln) -> int:
        # Count nearby peer items aligned in a vertical list.
        x_ref = ln.x0
        y_ref = ln.y0
        sz_ref = getattr(ln, "size", 0.0)
        cnt = 0
        for peer in black:
            if peer is ln:
                continue
            if abs(peer.x0 - x_ref) > 18.0:
                continue
            if abs(getattr(peer, "size", 0.0) - sz_ref) > 1.6:
                continue
            if abs(peer.y0 - y_ref) > 95.0:
                continue
            pt = _normalize_spaces((peer.text or "").strip())
            if not pt or _is_bracket_line(pt) or _looks_like_filled_value(pt) or _is_mostly_numeric(pt):
                continue
            if len(pt) > 55:
                continue
            cnt += 1
        return cnt

    filtered = []
    for f in fields:
        nf = _normalize_spaces((f or "").strip())
        ln = by_text.get(nf)
        if not ln:
            filtered.append(f)
            continue

        t = nf
        if not t:
            continue
        if t.endswith("?") or t.endswith(":"):
            filtered.append(f)
            continue
        if "(" in t or ")" in t:
            # Keep explanatory parentheticals (often real field labels).
            filtered.append(f)
            continue

        L = len(t)
        sz = getattr(ln, "size", 0.0)
        bold = getattr(ln, "bold", False)

        # Candidate option items are short, small, not bold, mid-page or lower, in list-like clusters,
        # and preceded by a parent prompt.
        if (L <= 28) and (not bold) and (sz <= 11.6) and (ln.y0 >= min(0.22 * height, 150.0)):
            if has_parent_prompt(ln) and listlike_density(ln) >= 2:
                continue

        filtered.append(f)

    return filtered


def _looks_like_filled_value(t: str) -> bool:
    s = (t or "").strip()
    if not s:
        return False

    if _DATEISH_RE.search(s) or _TIMEISH_RE.search(s):
        return True

    alnum = sum(ch.isalnum() for ch in s)
    digits = sum(ch.isdigit() for ch in s)
    if alnum >= 6 and digits / max(1, alnum) >= 0.55:
        return True

    if len(s) > 55 and (s.count(";") >= 2 or s.count(",") >= 4):
        return True

    return False


def _contains_checkbox_glyph(t: str) -> bool:
    glyphs = {"□", "☐", "▢", "▣", "■", "☑", "✓", "✔"}
    return any(ch in glyphs for ch in (t or ""))


def _clean_field_names(fields: List[str]) -> List[str]:
    cleaned = []
    for f in fields:
        t = _normalize_spaces((f or "").strip())
        if not t:
            continue
        if _is_bracket_line(t):
            continue
        if _MOSTLY_PUNCT_RE.match(t):
            continue
        if _is_mostly_numeric(t):
            continue
        if len(t) == 1 and _is_bullet_char(t):
            continue
        t = re.sub(r"\s+-\s*$", "", t).strip()
        t = _ENUM_PREFIX_RE.sub("", t).strip()
        if not t:
            continue
        cleaned.append(t)
    return cleaned


def _dedup_preserve_order(items: List[str]) -> List[str]:
    seen = set()
    out = []
    for it in items:
        if it in seen:
            continue
        seen.add(it)
        out.append(it)
    return out


def _normalize_spaces(s: str) -> str:
    return re.sub(r"\s+", " ", s or "").strip()


def _is_bracket_line(t: str) -> bool:
    t = (t or "").strip()
    return bool(_BRACKET_ANY_RE.match(t))


def _is_mostly_numeric(t: str) -> bool:
    s = (t or "").strip()
    if not s:
        return True
    alnum = sum(ch.isalnum() for ch in s)
    digits = sum(ch.isdigit() for ch in s)
    if alnum == 0:
        return True
    return digits >= max(1, int(0.80 * alnum))


def _is_bullet_char(ch: str) -> bool:
    return ch in {"•", "·", "‣", "▪", "●", "○", "◯", "–", "-", "•"}
```
