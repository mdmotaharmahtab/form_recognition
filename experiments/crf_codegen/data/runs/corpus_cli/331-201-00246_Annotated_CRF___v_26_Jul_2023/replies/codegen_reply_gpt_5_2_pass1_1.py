```python
# Observed layouts: (1) form pages where each entry field has a red bracketed code line like "[XXXX]" beneath a human label,
# often with red "[TYPE: ...]" metadata; (2) matrix/table pages where the label is in a left column and entries are in
# right columns under printed column headers; (3) non-field pages (TOC/copyright) lacking bracketed code markers.
# Strategy: detect/carry forward a prominent top-left title as form_name; for each red bracketed code, backtrack to the
# nearest human label block by geometry, merge wrapped lines, and append a column header when the entry sits in a right column.
...
def extract(pages: List[Tuple[int, list]]) -> List[Dict[str, object]]:
    ...
    for page_idx0, lines in pages:
        ...
        # Collect field code lines (red bracket codes)
