...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        # Family C: schedule tables with Study Event / Form columns
        # Extract form names from the "Form" column (x~165)
        if any(ln.text.strip() == "Study Event" and ln.x0 < 60 for ln in lines):
            form_col_x = None
            for ln in lines:
                if ln.text.strip() == "Form" and 150 < ln.x0 < 180 and ln.y0 < 70:
                    form_col_x = ln.x0
                    break
            if form_col_x:
                for ln in lines:
                    if (abs(ln.x0 - form_col_x) < 20 and ln.y0 > 70 and 
                        not ln.non_black and ln.size < 10):
                        form_text = ln.text.strip()
                        # Filter junk: empty, "Conditionally Visible", technical codes
                        if (form_text and len(form_text) > 2 and 
                            "Conditionally" not in form_text and 
                            "Form:" not in form_text and
                            "Study Event:" not in form_text and
                            not re.match(r'^(Day|Timepoint|Conditional)', form_text)):
                            results.append({
                                "form_name": form_text,
