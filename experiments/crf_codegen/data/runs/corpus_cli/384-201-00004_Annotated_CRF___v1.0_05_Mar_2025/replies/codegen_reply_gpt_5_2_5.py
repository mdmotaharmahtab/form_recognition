...
_RX_WIDGET_PIPE_UNDERSCORE = re.compile(r"\[\s*[_\|]{2,}\s*\]")
...
def extract(pages) -> List[Dict[str, str]]:
    ...
    def _get(ln: Any, name: str, default=None):
        if isinstance(ln, dict):
            return ln.get(name, default)
        return getattr(ln, name, default)

    def _text(ln) -> str:
        return (_get(ln, "text", "") or _get(ln, "t", "") or "")

    def _size(ln) -> float:
        return _safe_float(
            _get(ln, "size", None)
            if _get(ln, "size", None) is not None
            else _get(ln, "sz", None)
            if _get(ln, "sz", None) is not None
            else _get(ln, "font_size", None),
            0.0,
        )
    ...
    def _x0(ln) -> float:
        return _safe_float(
            _get(ln, "x0", None)
            if _get(ln, "x0", None) is not None
            else _get(ln, "x", None)
            if _get(ln, "x", None) is not None
            else _get(ln, "left", None),
            0.0,
        )
    ...
    def _y0(ln) -> float:
        return _safe_float(
            _get(ln, "y0", None)
            if _get(ln, "y0", None) is not None
            else _get(ln, "y", None)
            if _get(ln, "y", None) is not None
            else _get(ln, "top", None),
            0.0,
        )

    def _approx_x1_from_text(ln) -> float:
        x = _x0(ln)
        sz = _size(ln)
        t = _text(ln) or ""
        # conservative width estimate; only used when true x1 is missing
        return x + max(1.0, 0.52 * sz * max(1, len(t)))

    def _x1(ln) -> float:
        v = _get(ln, "x1", None)
        if v is None:
            v = _get(ln, "right", None)
        if v is None:
            return _approx_x1_from_text(ln)
        vx1 = _safe_float(v, 0.0)
        vx0 = _x0(ln)
        if vx1 <= vx0 + 0.5:
            return _approx_x1_from_text(ln)
        return vx1
    ...
    def _y1(ln) -> float:
        v = _get(ln, "y1", None)
        if v is None:
            v = _get(ln, "bottom", None)
        if v is None:
            return _y0(ln) + max(1.0, 1.05 * _size(ln))
        vy1 = _safe_float(v, 0.0)
        vy0 = _y0(ln)
        if vy1 <= vy0 + 0.5:
            return vy0 + max(1.0, 1.05 * _size(ln))
        return vy1
    ...
    def _is_bold(ln) -> bool:
        ...
        if b is not None:
            return bool(b)
        b = _get(ln, "is_bold", None)
        if b is not None:
            return bool(b)
        font = str(_get(ln, "font", "") or _get(ln, "fontname", "") or "")
        if font:
            f = font.lower()
            if "bold" in f or "demi" in f or "black" in f:
                return True
        return False

    def _color_value(ln):
        return _get(ln, "color", None) if _get(ln, "color", None) is not None else _get(ln, "fill", None)
    ...
    def _is_non_black(ln) -> bool:
        ...
        if isinstance(c, (tuple, list)) and len(c) >= 3:
            # rgb-ish (0..1 or 0..255)
            try:
                r, g, b = float(c[0]), float(c[1]), float(c[2])
                if r == g == b == 0:
                    return False
                if r <= 1.0 and g <= 1.0 and b <= 1.0 and r == g == b and r < 0.05:
                    return False
                return True
            except Exception:
                return False
        ...
        if not s:
            return False
        if s in ("black", "#000", "#000000", "0", "0.0"):
            return False
        # treat any explicit non-black hex as non_black
        if s.startswith("#") and s not in ("#000", "#000000"):
            return True
        # named colors other than "black"
        if s.isalpha() and s != "black":
            return True
        return False
    ...
    def _page_wh(lines) -> Tuple[float, float]:
        ...
        max_y = 0.0
        for ln in lines:
            max_x = max(max_x, _x0(ln), _x1(ln))
            max_y = max(max_y, _y0(ln), _y1(ln))
        if max_x < 200:
            max_x = 612.0
        if max_y < 200:
            max_y = 792.0
    ...
    def _looks_like_widget(text: str) -> bool:
        ...
        if _RX_WIDGET_BOX.search(t):
            return True
        if _RX_WIDGET_PIPE_UNDERSCORE.search(t):
            return True
    ...
    def _is_technical_annotation_line(ln, w: float, h: float, base_sz: float) -> bool:
        s = (_text(ln) or "").strip()
        if not s:
            return True
        x = _x0(ln)
        sz = _size(ln)
        # Right-side narrow technical column tends to be far right + smaller font.
        if x > 0.62 * w and sz > 0 and sz <= max(6.4, base_sz - 0.6):
            return True
        # Very small font anywhere is usually annotation/help text (not labels/headers).
        if sz > 0 and sz <= max(5.8, base_sz - 1.4) and _count_letters(s) < 10:
            return True
        return False
    ...
    def _page_left_xmax(lines) -> float:
        ...
        for ln in lines:
            ...
            if _looks_like_widget(s):
                continue
            if _is_technical_annotation_line(ln, w, h, base_sz):
                continue
            if _y0(ln) > 0.93 * h:
                continue
            if _x0(ln) > 0.70 * w:
    ...
    def _collect_widgets(lines):
        ...
        for ln in lines:
            ...
            if _looks_like_widget(s):
                widgets.append(ln)
        widgets.sort(key=lambda z: (_y0(z), _x0(z)))
        return widgets
    ...
    def _looks_like_approval_page(lines) -> bool:
        ...
        for ln in lines:
            ...
            if _is_technical_annotation_line(ln, w, h, base_sz):
                continue
            if _y0(ln) < top_band and _is_bold(ln) and _size(ln) >= 18:
                if _x0(ln) < 0.85 * w and _count_letters(s) >= 4:
                    return True
        return False

    def _join_title_band(lines, anchor_ln, y_slack: float = 20.0, x_slack: float = 70.0) -> str:
        base_x = _x0(anchor_ln)
        base_y = _y0(anchor_ln)
        base_sz = _size(anchor_ln)
        ...
        for ln in lines:
            ...
            if not _is_title_candidate_text(s):
                continue
            if abs(_size(ln) - base_sz) > 2.4:
                continue
            if abs(_y0(ln) - base_y) > y_slack:
                continue
            if abs(_x0(ln) - base_x) > x_slack:
                continue
            band.append(ln)

        band.sort(key=lambda z: (_y0(z), _x0(z)))
        parts = [(_text(ln) or "").strip() for ln in band if (_text(ln) or "").strip()]
    ...
    def _find_form_title(lines, has_fields: bool, is_approval: bool) -> Optional[str]:
        ...
        if is_approval:
            top = [ln for ln in lines if (_text(ln) or "").strip() and _y0(ln) < 0.28 * h]
            if not top:
                return None
    ...
    def _cluster_by_proximity(items, y_gap: float, x_gap: float) -> List[List]:
        if not items:
            return []
        items = sorted(items, key=lambda z: (_y0(z), _x0(z)))
        groups: List[List] = []
        cur = [items[0]]
        for it in items[1:]:
            py = _y0(cur[-1])
    ...
    def _best_label_above(lines, anchor, max_up: float, x_center: float, x_span: float, w: float, h: float, base_sz: float) -> Optional[str]:
        ay0 = _y0(anchor)
        best_ln = None
        best_score = -1e9
        for ln in lines:
    ...
    def _best_label_left(lines, anchor, left_xmax: float, y_slack: float, x_slack: float, w: float, h: float, base_sz: float) -> Optional[str]:
        ax0 = _x0(anchor)
        ay0 = _y0(anchor)
        candidates = []
        for ln in lines:
            ...
            if abs(y0 - ay0) > y_slack:
                continue
            if _is_bold(ln) and _size(ln) >= _size(anchor) + 4.0:
                continue
            candidates.append(ln)

        if not candidates:
            return None
        candidates.sort(key=lambda z: (_y0(z), _x0(z)))
    ...
    def _section_headers(lines) -> List:
        ...
        hs.sort(key=lambda z: (_y0(z), _x0(z)))
        return hs
    ...
    def _extract_radio_option_groups(lines, base_sz: float) -> List[Tuple[float, float, float, float, object]]:
        ...
        out = []
        for g in groups:
            ys = [_y0(ln) for ln in g]
            xs0 = [_x0(ln) for ln in g]
            xs1 = [_x1(ln) for ln in g]
            y0 = min(ys) if ys else 0.0
            y1 = max(ys) if ys else y0
            x0 = min(xs0) if xs0 else 0.0
            x1 = max(xs1) if xs1 else x0
            g_sorted = sorted(g, key=lambda z: (_y0(z), _x0(z)))
            out.append((x0, y0, x1, y1, g_sorted[0]))
        out.sort(key=lambda z: (z[1], z[0]))
        return out

    def _is_footer_or_header_furniture(ln, w: float, h: float, base_sz: float) -> bool:
        s = (_text(ln) or "").strip()
        if not s:
            return True
        y0 = _y0(ln)
        ...
        # very small right-column items are almost always technical furniture/annotation
        if _is_technical_annotation_line(ln, w, h, base_sz):
            return True
    ...
    for page in pages or []:
        lines = list(_get(page, "lines", None) or _get(page, "spans", None) or _get(page, "blocks", None) or [])
        ...
        for ln in lines:
            ...
            if not s:
                continue
            if _is_footer_or_header_furniture(ln, w, h, base_sz):
                continue
            usable_lines.append(ln)

        widgets = _collect_widgets(usable_lines)
        has_fields = bool(widgets) or bool(_extract_radio_option_groups(usable_lines, base_sz=base_sz))
        ...
        # 1) Option groups -> one field per question.
        ...
        # 3) Explicit "Label:" without a visible widget (guarded to avoid filled values).
        for ln in usable_lines:
            s = (_text(ln) or "").strip()
            if not s:
                continue
            ...
            if x0 > left_xmax + 0.10 * w:
                continue
            if _is_bold(ln) and _size(ln) >= base_sz + 2.5:
                continue
            ...
            for ln2 in usable_lines:
                ...
                if x02 > x1 + max(2.2 * base_sz, 0.04 * w) and x02 < x1 + 0.25 * w:
                    if _count_alnum(_text(ln2)) >= 3 and not _is_technical_annotation_line(ln2, w, h, base_sz):
