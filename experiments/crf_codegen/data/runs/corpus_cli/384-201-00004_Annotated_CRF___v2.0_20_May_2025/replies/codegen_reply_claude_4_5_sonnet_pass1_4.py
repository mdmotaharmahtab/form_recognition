Looking at the issues, I need to analyze why certain fields are being missed:

1. **Page 40** - Missing fields from C-SSRS form
2. **Page 56** - Missing fields from concomitant medications
3. **Cluster 1** (94 pages, 0% coverage) - These appear to be reference/mapping pages with blue links and code lists, which should be skipped (correct behavior)

Let me examine the sample pages more carefully:

- **Page 48** (cluster 0, extracted) shows the C-SSRS form with fields being captured
- **Page 140** (cluster 0, extracted) shows urine collection fields

The main issue seems to be that some field labels are being filtered out incorrectly by my skip patterns or validation rules. Let me trace through what might cause the misses on pages 40 and 56.
    ...
    for page_idx, lines in pages:
        ...
        for i, ln in enumerate(lines):
            if (not ln.non_black and 
                ...
                # Must have some alphabetic content (relaxed: allow single letter words)
