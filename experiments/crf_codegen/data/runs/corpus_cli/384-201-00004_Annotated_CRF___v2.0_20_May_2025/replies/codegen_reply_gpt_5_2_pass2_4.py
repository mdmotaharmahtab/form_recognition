...
def extract(pages: List[Tuple[int, List[Any]]]) -> List[Dict[str, Any]]:
    ...
    for page_idx0, lines in pages:
        ...
        # DocuSign certificate/audit pages: extract human signer + role labels (only).
        if docusign_kind == "certificate":
            form_name = (current_form or "").strip()
            if not form_name:
                form_name = _infer_form_name_from_docusign_subject(lines)

            labels = _extract_signature_name_role_labels(lines, max_y1)
            if labels:
                seen = set()
                for lab in labels:
                    key = (form_name, lab)
...
def _classify_docusign_page(lines: List[Any]) -> str:
    """
    Returns:
      - "certificate": DocuSign Certificate of Completion / audit-style pages
      - "disclosure": narrative Electronic Record and Signature Disclosure pages
      - "": not DocuSign-related (for our purposes)
    """
    # Certificate/audit pages: table-like headers or explicit certificate markers.
    ...
    # Disclosure pages: big all-caps heading and long paragraphs (no fields).
...
def _infer_form_name_from_docusign_subject(lines: List[Any]) -> str:
    # Try to pull the underlying document name from "Subject:".
    subj = ""
    for ln in lines:
        t = _norm(_txt(ln))
        if not t:
        ...
        if t.lower().startswith("subject:"):
            subj = _norm(t.split(":", 1)[1])
            break
    if not subj:
        return ""

    m = re.search(r"\b([A-Za-z]*CRF\b[^,]{0,50}\bApproval Form)\b", subj, flags=re.IGNORECASE)
    if m:
        return _norm(m.group(1))

    # Fallback: look for "... Approval Form ..." chunk.
    m2 = re.search(r"\b(.{0,40}Approval Form)\b", subj, flags=re.IGNORECASE)
    if m2:
        cand = _norm(m2.group(1))
        if 6 <= len(cand) <= 80 and _meaningful_text(cand):
            return cand
