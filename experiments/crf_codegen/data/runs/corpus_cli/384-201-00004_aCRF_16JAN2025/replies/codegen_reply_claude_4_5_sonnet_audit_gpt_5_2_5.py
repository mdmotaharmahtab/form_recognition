...
1. **Fix Page 91**: Missing "Comments" field - likely a simple label that my heuristics are too strict for
2. **Handle Cluster 1 (71 pages, 0% coverage)**: These are code-list/reference tables with "Coded"/"Decode" headers - I'm correctly skipping them (they contain answer options, not data-entry fields)
3. **Handle Cluster 3 (7 pages, 0% coverage)**: These are field definition metadata pages with only right-column technical annotations - I'm correctly skipping them (no actual fields)

The main issue is the missing "Comments" field on page 91. I need to relax my field detection heuristics to catch short, simple labels like "Comments".
...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            # RELAXED: Allow slightly longer labels and be more permissive
            if not is_field:
                if (len(full_label) <= 20 and 
                    full_label[0].isupper() and 
                    not re.match(r'^[A-Z]{6,}$', full_label)):  # Not long codes
                    # Additional check: looks like a sensible noun/field name
                    # (not a sentence fragment, not all numbers)
                    if not re.search(r'\d{3,}', full_label):  # No long number sequences
                        is_field = True
            ...
            # Catch standalone common field names that might have been missed
            # (like "Comments" which is a very common field label)
            if not is_field:
                common_field_names = [
                    'comments', 'notes', 'remarks', 'pr', 'qrs', 'qt', 
                    'qtc', 'qtcf', 'rr', 'hr', 'bp', 'weight', 'height',
                    'temperature', 'pulse', 'respiration'
                ]
                if full_label.lower() in common_field_names:
                    is_field = True
