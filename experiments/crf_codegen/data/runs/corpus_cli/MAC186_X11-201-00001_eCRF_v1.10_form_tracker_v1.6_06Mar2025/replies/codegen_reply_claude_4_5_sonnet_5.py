Looking at the quality issues, I need to:

1. **Fix cluster 6 (visit title pages)** - currently returning empty results, which is correct (they have no fields)
2. **Fix cluster 1 (95% → 100%)** - the uncovered pages show a continuation pattern from previous pages
3. **Fix false extractions** (Page 16: export codes like CSS0409B being extracted as fields)
4. **Fix missed fields** (Page 25, 163, 195: legitimate fields not being captured)
5. **Remove all literal string blocklists** - rely only on structural rules

Let me analyze the sample pages:
- Pages 369, 615: These show field labels with [N] codes - standard form pages that should already work
- Pages 374, 618: Visit title pages - correctly returning nothing
- Page 16 issue: Export codes (CSS0409B) are being extracted - need better filtering in `extract_export_code_definitions`
- Page 163 issue: Complex vitals page with mixed layouts
- Page 195 issue: Numbered items with text wrapping issues
