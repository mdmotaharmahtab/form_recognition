...
def extract(pages: List[Tuple[int, List[Any]]]) -> List[Dict[str, Any]]:
    ...
    for page_idx, raw_lines in pages:
        ...
        # Skip variable-definition/spec listing pages (not data-entry forms)
        if _is_variable_definition_listing(rows, min_x=min_x, x_span=x_span):
            continue

        fields: List[str] = []

        # Layout dispatch (extend, do not remove existing behavior)
...
def _looks_like_short_code(s: str) -> bool:
    """
    Shorter structural code tokens (used for page-type detection only).
    """
    s = _norm(s)
    if not s or " " in s:
        return False
    ...
    if not any(c.isalpha() for c in s) or not any(c.isdigit() for c in s):
        return False
    alnum = sum(c.isalnum() for c in s)
    if alnum / max(1, len(s)) < 0.9:
        return False
    # Many variable codes are all-caps-ish; allow a few lowercase but bias toward uppercase presence
...
def _is_variable_definition_listing(rows: List[List[Any]], min_x: float, x_span: float) -> bool:
    """
    Detect pages that list variables/codes with types/lengths/categories (spec pages),
    often containing many bracketed IDs + machine-ish variable codes and type/length columns.
    These are not data-entry CRF forms and should produce zero extracted fields.
    """
    if not rows:
        return False
    ...
    # Column regions relative to page width (slack)
    left_bracket_max = min_x + 0.18 * x_span
    code_col_min = min_x + 0.22 * x_span
    code_col_max = min_x + 0.55 * x_span
    type_col_min = min_x + 0.52 * x_span
    len_col_min = min_x + 0.64 * x_span
    len_col_max = min_x + 0.88 * x_span
    ...
    for row in rows:
        ...
        for ln in row:
            ...
            if code_col_min <= x <= code_col_max:
                ...
                if t2 and not _looks_like_machine_tracker(t2) and _looks_like_short_code(t2):
                    has_code = True

            if x >= type_col_min:
                t2 = _strip_bracket_codes(t)
                # "type" descriptors often contain parentheses and letters, not just numbers
                if "(" in t2 and ")" in t2 and _has_letters_or_words(t2) and 8 <= len(t2) <= 60:
                    has_type = True

            if len_col_min <= x <= len_col_max:
                if _is_pure_number(t) and len(t) <= 5:
                    has_len = True

        if has_code and (has_type or has_len):
            hit_rows += 1

    # Strong signal: many rows of bracketed IDs plus machine-like code/type structure
    if bracket_rows >= 8 and hit_rows >= max(6, int(0.65 * bracket_rows)):
        return True
...
# ----------------------------
# Questionnaire/list pages (cluster 8) - thresholds loosened to match page scale
# ----------------------------
    ...
    for row in rows:
        ...
        for ln in row:
            ...
            if x <= min_x + 0.085 * x_span and _is_pure_number(t):
                has_left_num = True
            # loosen text column start slightly (some pages have smaller left gutter)
            if (min_x + 0.075 * x_span) <= x <= (min_x + 0.78 * x_span):
                t2 = _strip_bracket_codes(t)
...
def _extract_questionnaire_list_fields(rows: List[List[Any]], min_x: float, x_span: float) -> List[str]:
    ...
    for row in rows:
        ...
        for ln in row:
            ...
            in_text_col = (min_x + 0.075 * x_span) <= x <= (min_x + 0.82 * x_span)
...
# ----------------------------
# Repeating schedule tables (cluster 7) - require indexed rows to avoid headers
# ----------------------------
...
def _extract_schedule_repeat_table_fields(rows: List[List[Any]], min_x: float, x_span: float) -> List[str]:
    ...
    # Prefer right-side measurement label column(s); still allow mid labels if the row is indexed.
    ...
    for row in rows:
        ...
        # Only consider indexed data rows (prevents extracting header labels like "Repetition Number", etc.)
        has_left_index = any(
        ...
        for ln in row:
            ...
            # Prefer right-most measurement labels; allow mid only if it looks label-like (multiword / longer).
            if not in_right and not (in_mid and (t2.count(" ") >= 1 or len(t2) >= 12)):
                continue
...
# ----------------------------
# Header-band (multi-column) extraction (existing)
# ----------------------------
...
# ----------------------------
# Body label extraction - extended to recover non-bold labels, with option-list suppression
# ----------------------------
...
def _extract_body_label_fields(
    ...
    def ok_label_text(t2: str) -> bool:
        ...
        if _looks_like_machine_tracker(t2) or _looks_like_code_token(t2):
            return False
        # Avoid tiny fragments that are usually checkbox/radio option tokens
        if len(t2) <= 2:
            return False
        return True

    # Collect candidates (bold + non-bold) to infer label columns robustly.
    cand_xs: List[float] = []
    cand_lns: List[Any] = []
    ...
    for row in rows:
        for ln in row:
            ...
            sz = float(getattr(ln, "size", 0.0) or 0.0)
            if max_size > 0 and sz >= 0.92 * max_size:
                continue

            bold = bool(getattr(ln, "bold", False))
            # Candidate evidence: bold OR label-like (multiword / endswith colon / longer)
            ev = bold or t2.endswith(":") or (t2.count(" ") >= 1 and len(t2) >= 8) or (len(t2) >= 14)
            if not ev:
                continue
    ...
    # Option-list filter: detect indented vertical choice lists under a parent label (bold OR non-bold).
    def is_option_like(row_i: int, ln: Any) -> bool:
        x = float(getattr(ln, "x0", 0.0) or 0.0)
        y = float(getattr(ln, "y0", 0.0) or 0.0)
        # slightly broader indent window (covers long option lists on forms)
        if not (base_left + 4.0 <= x <= base_left + 52.0):
            return False

        # Need a nearby parent label above at (near) base_left.
        parent_found = False
        for up in range(row_i - 1, max(-1, row_i - 12), -1):
            for up_ln in rows[up]:
                up_y = float(getattr(up_ln, "y0", 0.0) or 0.0)
                if y - up_y > 62.0:
                    break
                up_x = float(getattr(up_ln, "x0", 0.0) or 0.0)
                if abs(up_x - base_left) <= 20.0:
                    up_txt = _strip_bracket_codes(_text(up_ln))
                    if ok_label_text(up_txt) and (up_txt.count(" ") >= 1 or len(up_txt) >= 10):
                        parent_found = True
                        break
            if parent_found:
                break
        ...
        sibs = 0
        for dn in range(row_i + 1, min(len(rows), row_i + 22)):
    ...
    for i, row in enumerate(rows):
        for ln in row:
            ...
            # Suppress option list items
            if is_option_like(i, ln):
                continue

            # Extra suppression: very short, non-bold tokens in indent region are typically options
            if not bool(getattr(ln, "bold", False)) and (base_left + 4.0 <= x <= base_left + 52.0):
                if len(t2) <= 6 and t2.count(" ") == 0:
                    continue
            ...
            while j < len(rows):
                ...
                if cont_y - last_y > max(11.5, 2.35 * (sz if sz > 0 else 7.0)):
                    break
                ...
                for nn in next_row:
                    ...
                    if not ok_label_text(nt2):
                        continue
                    if (base_left + 4.0 <= nx <= base_left + 52.0) and is_option_like(j, nn):
                        continue
                    cont_txt = nt2
                    cont_x = nx
                    break
                ...
                # Avoid swallowing a new label that happens to align but is clearly a separate record (large vertical jump already handled)
