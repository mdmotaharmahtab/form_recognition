Looking at the issues, I need to fix the program to filter out non-data-entry fields that are being incorrectly extracted:

1. **Page 3**: Multi-line question split incorrectly
2. **Page 6**: Instruction text "Please ensure that a Protocol Deviation is completed"
3. **Page 9**: Instruction text "Please ensure that an appropriate Medical History Quicklist is completed"
4. **Page 94**: "Confirm TDL form completed:" - this might be a section header, not a field
5. **Page 156**: Mixed issues - date/time transcription notes and sample handling instructions

The main problems are:
- Instruction text starting with "Please ensure"
- Parenthetical transcription instructions like "(Transcribe from paper source"
- Sample handling instructions that mix with field names
- Multi-line fields being split incorrectly
    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            if (line.bold and 
                ...
                # Remove parenthetical transcription instructions like "(Transcribe from paper source"
                # These can appear mid-field and should be removed entirely
                field_name = re.sub(r'\s*\(Transcribe from[^)]*\)', '', field_name, flags=re.IGNORECASE)
                ...
                # Skip instruction text starting with "Please ensure"
                if re.match(r'^Please ensure that', field_name, re.IGNORECASE):
                    i = j
                    continue
                
                # Skip sample handling instructions (contain "invert" or "protect the sample")
                if re.search(r'\binvert\b.*\btimes\b', field_name, re.IGNORECASE):
                    i = j
                    continue
                
                if re.search(r'\bprotect the sample\b', field_name, re.IGNORECASE):
                    i = j
                    continue
                ...
                # But allow "Confirm TDL form completed:" which is a field - actually no, this is NOT a field based on audit
                if field_name.endswith(":"):
                    ...
                    if ":" in field_name[:-1]:
                        ...
                        i = j
                        continue
                    elif (not field_name.startswith("Please") and
                          not field_name.startswith("Record") and
                          not "?" in field_name):
                        # Single colon at end, not a clear question - likely header
                        # But keep fields starting with action words like "Please", "Record"
                        # Remove "Confirm" from the exception list - "Confirm TDL form completed:" is not a field
                        i = j
                        continue
