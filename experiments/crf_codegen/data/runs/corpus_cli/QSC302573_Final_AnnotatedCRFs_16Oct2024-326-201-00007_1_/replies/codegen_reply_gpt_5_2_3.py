    ...
    for page_idx0, lines in pages:
        ...
        if ans_markers:
            for i, ans in enumerate(ans_markers):
                y_prev = header_cutoff_y if i == 0 else max(
                    header_cutoff_y, float(getattr(ans_markers[i - 1], "y0", header_cutoff_y))
                )
                ...
                # Checkbox-style subfields inside the answer region (includes common checkbox glyphs).
                ...
                # Radio-option style groups within the block (label + options), without extracting options.
        ...
        # --------
        # Pass 2: Underline/leader-like fill areas (fallback)
        # Extended: also supports labels stacked above the blank, and avoids dropping real left-margin labels
        # (e.g., Staff Initials, Comment) when they have nearby fill evidence.
        # --------
...
def _header_cutoff_y(lines, page_box, default_frac: float = 0.18) -> float:
    ...
    body_candidates = [
        float(getattr(ln, "y0", 0.0))
        for ln in lines
        if float(getattr(ln, "y0", 0.0)) >= (min_y + 0.12 * _h(page_box))
    ]
    if body_candidates:
...
def _x_overlap_ratio(a0: float, a1: float, b0: float, b1: float) -> float:
    ...
    ov = max(0.0, hi - lo)
    aw = max(1.0, abs(a1 - a0))
    bw = max(1.0, abs(b1 - b0))
...
def _is_bullet_line(txt: str) -> bool:
    t = _norm_text(txt)
    if not t:
        return False
...
def _looks_like_instruction_bullet(txt: str) -> bool:
    # Suppress checklist/instruction bullets (e.g., "- Correct ...") unless they are explicitly
    # formatted as checkbox/radio lines.
    t = _norm_text(txt)
    if not _is_bullet_line(t):
        return False
    if _is_checkbox_line(t) or _is_option_line(t):
        return False
    # If it looks like a label (ends with ":"), allow; otherwise treat as non-field.
    if re.search(r":\s*$", t):
        return False
    if _letters_count(t) >= 6 and len(t) <= 140:
        return True
    return False
...
def _is_option_line(txt: str) -> bool:
    t = _norm_text(txt)
    # Accept common radio glyphs as option markers.
    return bool(re.match(r"^\s*(?:[Oo]|○|◯|◉|●)\s+\S+", t))


def _is_checkbox_line(txt: str) -> bool:
    t = _norm_text(txt)
    if re.match(r"^\s*\[\s*[xX]?\s*\]\s*\S+", t):
        return True
...
def _estimate_label_column_x(lines, ans_markers, page_box, header_cutoff_y: float) -> float:
    ...
    for ln in lines:
        ...
        if _looks_like_placeholder(txt):
            continue
        if _looks_like_instruction_bullet(txt):
            continue
...
def _has_fill_evidence_near_label(label_ln, lines, page_box) -> bool:
    # If a blue/bold left-margin label has a nearby blank/underline to its right or just below,
    # it's likely a real data-entry field label (e.g., Staff Initials, Comment), not furniture.
    y = float(getattr(label_ln, "y0", 0.0))
    x0 = float(getattr(label_ln, "x0", 0.0))
    x1 = float(getattr(label_ln, "x1", x0))
    for ln in lines:
        if ln is label_ln:
            continue
        y2 = float(getattr(ln, "y0", 0.0))
        if abs(y2 - y) <= 3.0 or (0.0 <= (y2 - y) <= 22.0):
            txt2 = _norm_text(getattr(ln, "text", ""))
            if not txt2 or not _is_underline_line(txt2):
                continue
            ux0 = float(getattr(ln, "x0", 0.0))
            ux1 = float(getattr(ln, "x1", ux0))
            # right-of-label or aligned beneath/overlapping indicates fill area
            if ux0 >= (x1 - 2.0) or _x_overlap_ratio(x0, x1, ux0, ux1) >= 0.35:
                return True
    return False


def _is_probable_furniture_label(ln, lines, page_box, header_cutoff_y: float, ans_markers) -> bool:
    y0 = float(getattr(ln, "y0", 0.0))
    if y0 < header_cutoff_y:
        return True
    ...
    if _has_fill_evidence_near_label(ln, lines, page_box):
        return False
...
def _extract_main_label(lines, label_x: float, y0: float, ans_y: float, page_box, ans_markers) -> str:
    ...
    for ln in lines:
        ...
        if _is_probable_furniture_label(ln, lines, page_box, y0, ans_markers):
            continue
    ...
    if _looks_like_placeholder(label) or _looks_like_code_label(label) or _looks_incomplete_parenthetical(label):
        return ""
    if _looks_like_instruction_bullet(label):
        return ""
    return label
...
def _extract_checkbox_prompts_in_region(lines, answer_marker, y0: float, y1: float, label_x: float, page_box) -> List[str]:
    ...
    for ln in region:
        ...
        if _looks_like_placeholder(cleaned) or _looks_like_code_label(cleaned) or _looks_incomplete_parenthetical(cleaned):
            continue
        if _looks_like_instruction_bullet(cleaned):
            continue
...
def _extract_underline_fields(lines, page_box, header_cutoff_y: float, label_x: float, ans_markers) -> List[str]:
    ...
    for ln in lines:
        ...
        # Avoid extreme left margin, but don't drop legitimate blanks on pages where the label
        # column is itself left-margin (e.g., Staff Initials / Comment areas).
        if x0 < (min_x + 0.14 * width):
            continue

        # Drop "mask" underlines that live in the label column (left of the Answer(s) label column)
        # and are mostly structural.
        if x0 <= (label_x - 18.0) and txt.count("_") >= 6:
            continue
    ...
    for ul in underline_lines:
        ...
        # (A) label on the same row to the left of underline
        label_ln = None
        best_x = None
        for ln in row:
            if ln is ul:
            ...
            if _is_probable_furniture_label(ln, lines, page_box, header_cutoff_y, ans_markers):
                continue

            if _letters_count(txt) < 3:
                continue
        ...
        # (B) label slightly above, stacked/overlapping the underline column
        if label_ln is None:
            best = None
            best_score = None
            for ln in lines:
                ...
                # Allow either left-of-underline OR stacked above (x overlap) layout.
                overlap = _x_overlap_ratio(x0, x1, x0_ul, x1_ul)
                aligned = abs(x0 - x0_ul) <= 26.0 or overlap >= 0.35
                leftish = x1 <= (x0_ul - 6.0)

                if not (leftish or aligned):
                    continue
                ...
                # Score: prefer closest in y, then best x alignment to underline column
                score = (y - y2, abs(x0 - x0_ul), abs(x0 - label_x))
        ...
        # Join wrapped label lines above (same x band, allow slightly looser x if stacked layout)
        for ln in sorted(lines, key=lambda l: float(getattr(l, "y0", 0.0)), reverse=True):
            ...
            if abs(x0 - x0_base) > 14.0 and _x_overlap_ratio(x0, x1, x0_base, float(getattr(label_ln, "x1", x0_base))) < 0.35:
                continue
            ...
            if _is_underline_line(txt2) or _looks_like_placeholder(txt2):
                continue
            if _looks_like_instruction_bullet(txt2):
                continue
            if _letters_count(txt2) < 3:
                continue
...
def _extract_option_group_labels_pagewide(lines, page_box, header_cutoff_y: float, label_x: float, ans_markers) -> List[str]:
    ...
    for ln in opt_lines:
        ...
        if not cur:
            ...
            last_y, last_x = y, x
            continue
        if (y - last_y) <= 16.0 and abs(x - last_x) <= 22.0:
            cur.append(ln)
            last_y, last_x = y, x
        else:
            if len(cur) >= 2:
...
def _add(out, seen, form_name: str, field_name: str, page_num: int) -> None:
    ...
    # Normalize trailing colon if it sneaks in from some paths.
    fld = _strip_trailing_colon(fld)
    ...
    if _ANSWER_RE.match(fld):
        return
    if _looks_like_instruction_bullet(fld):
        return
