...
# Unit-ish hints; used primarily to trim trailing templates.
_UNIT_RE = re.compile(
    r"\b(mmHg|beats\s*/\s*min|breaths\s*/\s*(?:min|mi)|Degrees\s+Celsius|C\b)\b",
    re.IGNORECASE,
)
...
def _looks_like_measurement_label(t: str) -> bool:
    ...
    if any(k in tl for k in _MEAS_KEYS):
        return True
    if "drop" in tl and ("systolic" in tl or "diastolic" in tl):
        return True
...
def extract(pages: List[Tuple[int, List[Any]]]) -> List[Dict[str, Any]]:
    ...
    for page_index0, lines in pages:
        ...
        # Staff Initials / Comment markers can be real fields in this family.
        for ln in lines:
            if _looks_like_furniture(ln):
                continue
            y = getattr(ln, "y0", 0.0)
        ...
        # (e.g., Date/Time/Part/Version labels, short measurement labels, etc.).
