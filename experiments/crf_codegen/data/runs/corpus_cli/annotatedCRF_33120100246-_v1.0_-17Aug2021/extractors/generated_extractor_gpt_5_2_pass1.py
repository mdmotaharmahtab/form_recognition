...
def _looks_like_definition_sentence(t: str) -> bool:
    ...
    if "?" not in t and len(t) >= 80 and (t.count(";") >= 2 or (t.count(",") >= 4)):
        return True
    # Often-answer-option / explanatory blocks: long, punctuation-heavy, not a prompt.
    if "?" not in t and ":" not in t and len(t) >= 70 and (t.count(";") >= 1 or t.count("(") >= 2 or t.count('"') >= 1):
        return True
    return False
...
def _bare_phrase(t: str) -> bool:
    ...
    if len(words) <= 4 and len(t) <= 28 and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9\s\-/]*", t):
        return True
    return False


def _valueish_text(t: str) -> bool:
    """
    Likely a filled value or widget-adjacent token rather than a field label.
    Structural preference: short, not prompt-like, not obviously a question.
    """
    t = (t or "").strip()
    if not t:
        return True
    if _promptish_text(t):
        return False
    if len(t) <= 14 and t.count(" ") <= 2 and not _looks_like_choice_legend(t) and not _looks_like_definition_sentence(t):
        # If it's a timepoint string, it can be either a row header (left) or a filled value (right).
        # Caller decides using geometry.
        return True
    if len(t) <= 18 and t.count(" ") <= 1 and re.fullmatch(r"[A-Za-z][A-Za-z\-]*", t):
        return True
    return False
...
def _text_candidate(ln, small_sz: float) -> bool:
    t = (getattr(ln, "text", "") or "").strip()
    if _looks_like_chrome_or_empty(t):
        return False
    if t.startswith("["):
    ...
    if len(t) > 160:
        return False
    sz = _f(getattr(ln, "size", 0.0))
    # Slightly wider than before: prompts on some pages run a bit larger than body font.
    if not (max(5.8, small_sz - 1.9) <= sz <= (small_sz + 6.4)):
        return False
    return True
...
def _header_like(ln, small_sz: float) -> bool:
    ...
    if t.startswith("["):
        return False
    sz = _f(getattr(ln, "size", 0.0))
    if not (small_sz + 0.9 <= sz <= small_sz + 6.2):
        return False
    if len(t) > 90:
        return False
    return True
...
def _wrap_join(seed_ln, cands, small_sz: float, left_margin_x: float, x_max: float = None) -> str:
    """
    Join wrapped label lines beneath a seed line, keeping to the same left edge.
    """
    seed_t = _norm_text((getattr(seed_ln, "text", "") or "").strip())
    if not seed_t:
        return ""

    sx = _f(getattr(seed_ln, "x0", 0.0))
    sy = _f(getattr(seed_ln, "y0", 0.0))

    y_step_max = max(18.0, 1.95 * small_sz)
    x_slack = max(14.0, 1.45 * small_sz)

    parts = [(sy, sx, seed_t)]
    last_y = sy
    last_x = sx

    # Collect a few following lines that look like continuation.
    for _ in range(6):
        nxt = None
        nxt_score = None
        for ln in cands:
            ...
            if y <= last_y + 0.4:
                continue
            if y > last_y + y_step_max:
                continue
            x = _f(getattr(ln, "x0", 0.0))
            if abs(x - last_x) > x_slack:
                continue
            if x < left_margin_x - 8.0:
                continue
            if x_max is not None and x > x_max + 5.0:
                continue
            ...
            # Prefer closer in y; modestly prefer prompt-like continuation.
            score = (y - last_y) * 1.0 + abs(x - last_x) * 0.6
            score += -2.5 if _promptish_text(t) else 0.0
        ...
        nt = _norm_text((getattr(nxt, "text", "") or "").strip())
        if nt:
            parts.append((_f(getattr(nxt, "y0", 0.0)), _f(getattr(nxt, "x0", 0.0)), nt))
            last_y = _f(getattr(nxt, "y0", 0.0))
            last_x = _f(getattr(nxt, "x0", 0.0))
        else:
            break

    parts.sort(key=lambda p: (p[0], p[1]))
    joined = _norm_text(" ".join(p[2] for p in parts))
    return joined
...
def _same_row_left_anchor(label_lines, code_ln, left_margin_x: float, small_sz: float):
    ...
    for ln in label_lines:
        ...
        score += 22.0 if _looks_like_choice_legend(t) else 0.0
        score += 14.0 if _looks_like_definition_sentence(t) else 0.0
        score += 12.0 if too_close_to_widget else 0.0
        score += 14.0 if _is_optionish_shape(t, x, left_margin_x) else 0.0

        # Prefer real prompts strongly over value-ish fragments.
        score += -10.0 if _promptish_text(t) else 0.0
        score += 10.0 if (_valueish_text(t) and x > left_margin_x + 170.0) else 0.0
...
def _find_prompt_above(label_lines, code_ln, left_margin_x: float, small_sz: float):
    ...
    for ln in label_lines:
        ...
        score += -11.0 if _promptish_text(t) else 0.0
        score += 8.0 if _valueish_text(t) else 0.0
...
def _find_column_header_above(label_lines, code_ln, left_margin_x: float, small_sz: float):
    ...
    for ln in label_lines:
        ...
        score += 6.0 if _valueish_text(t) else 0.0
        score += -6.0 if (len(t) <= 26 and not _is_timepointish_text(t)) else 0.0
...
def _find_row_header_left(label_lines, code_ln, left_margin_x: float, small_sz: float):
    cy = _f(getattr(code_ln, "y0", 0.0))
    cx = _f(getattr(code_ln, "x0", 0.0))
    y_tol = max(9.5, 1.15 * small_sz)
    ...
    for ln in label_lines:
        ...
        dx = cx - x
        score = dx * 0.25 + abs(y - cy) * 2.0
        score += -12.0 if _is_timepointish_text(t) else 0.0
...
def _infer_field_label(lines, code_ln, small_sz: float, text_lines, left_margin_x: float, header_lines, two_col_split):
    cx = _f(getattr(code_ln, "x0", 0.0))
    cy = _f(getattr(code_ln, "y0", 0.0))

    anchor = _same_row_left_anchor(text_lines, code_ln, left_margin_x, small_sz)
    if anchor is not None:
        ...
        text = _gather_same_row_text(
            ...
            x_max=min(cx - 14.0, ax + 420.0),
        )
        text = _norm_text(text)

        if text:
            # Column header directly above this marker (table-like)
            colh_ln = _find_column_header_above(text_lines, code_ln, left_margin_x, small_sz)
            colh = _norm_text((getattr(colh_ln, "text", "") or "").strip()) if colh_ln is not None else ""

            # Row header to the left (table-like)
            rowh_ln = _find_row_header_left(text_lines, code_ln, left_margin_x, small_sz)
            rowh = _norm_text((getattr(rowh_ln, "text", "") or "").strip()) if rowh_ln is not None else ""

            # If the left "label" looks like a filled value in the right-side field area, prefer a prompt above.
            rightish = ax >= left_margin_x + 170.0

            if _is_timepointish_text(text):
                # Timepoint string can be a row header (left) or a filled value (right).
                if (not rightish) and colh and (not _is_timepointish_text(colh)) and (not _looks_like_choice_legend(colh)):
                    return _norm_text(f"{text} - {colh}")
                if rightish and colh and (not _is_timepointish_text(colh)):
                    return colh

                pab = _find_prompt_above(text_lines, code_ln, left_margin_x, small_sz)
                if pab is not None:
                    t2 = _wrap_join(pab, text_lines, small_sz, left_margin_x, x_max=cx + 20.0)
                    if t2 and not _looks_like_choice_legend(t2) and not _looks_like_definition_sentence(t2):
                        return t2

            # Value-like tokens (e.g., selected dropdown text) should not become field labels.
            if _valueish_text(text) and rightish:
                pab = _find_prompt_above(text_lines, code_ln, left_margin_x, small_sz)
                if pab is not None:
                    t2 = _wrap_join(pab, text_lines, small_sz, left_margin_x, x_max=cx + 40.0)
                    if t2 and (_promptish_text(t2) or _RE_TRAILING_COLON.search(t2) or len(t2) >= 18):
                        return t2
                if colh and (not _valueish_text(colh)) and (not _looks_like_choice_legend(colh)) and (not _looks_like_definition_sentence(colh)):
                    return colh

            # Question stem with column header (Lifetime / Past 3 Month, etc.)
            if _promptish_text(text) and colh and len(colh) <= 22 and (colh.lower() not in text.lower()):
                if not _looks_like_choice_legend(colh) and not _looks_like_definition_sentence(colh):
                    if not (_is_timepointish_text(colh) and rightish):
                        return _norm_text(f"{text} - {colh}")

            # If anchor is a bare phrase (often row label OR filled value), try prompt above before committing.
            if _bare_phrase(text) or _looks_like_definition_sentence(text) or _is_optionish_shape(text, ax, left_margin_x):
                pab = _find_prompt_above(text_lines, code_ln, left_margin_x, small_sz)
                if pab is not None:
                    t2 = _wrap_join(pab, text_lines, small_sz, left_margin_x, x_max=cx + 40.0)
                    if t2 and (_promptish_text(t2) or _RE_TRAILING_COLON.search(t2) or len(t2) >= 18):
                        if not _looks_like_choice_legend(t2) and not _looks_like_definition_sentence(t2):
                            return t2

            # Table-like: timepoint row header + PK column header
            if rowh and _is_timepointish_text(rowh) and (rowh_ln is not None):
                rx = _f(getattr(rowh_ln, "x0", 0.0))
                if rx <= left_margin_x + 130.0 and colh and not _is_timepointish_text(colh):
                    return _norm_text(f"{rowh} - {colh}")

            # Header context can disambiguate duplicates (keep old behavior)
            ...
            if hdr and (hdr.lower() not in text.lower()) and (not _looks_like_choice_legend(hdr)) and (not _is_timepointish_text(hdr)):
                if _bare_phrase(text) and two_col_split is None and (ax <= left_margin_x + 170.0):
                    return _norm_text(hdr)

            # If it looks like a wrapped prompt line, try to extend downward.
            if _promptish_text(text) and len(text) >= 18:
                # Find the actual line object matching anchor, and wrap-join from it.
                joined = _wrap_join(anchor, text_lines, small_sz, left_margin_x, x_max=cx + 40.0)
                if joined and len(joined) >= len(text):
                    return joined

            return text

    # No good same-row anchor: pick prompt above and join wrap.
    pab = _find_prompt_above(text_lines, code_ln, left_margin_x, small_sz)
    if pab is not None:
        base = _wrap_join(pab, text_lines, small_sz, left_margin_x, x_max=cx + 60.0)
        if base and not _looks_like_choice_legend(base) and not _looks_like_definition_sentence(base):
            # Optionally attach a short column header when marker sits in a table column.
            colh_ln = _find_column_header_above(text_lines, code_ln, left_margin_x, small_sz)
            colh = _norm_text((getattr(colh_ln, "text", "") or "").strip()) if colh_ln is not None else ""
            if colh and len(colh) <= 22 and (colh.lower() not in base.lower()):
                if not _looks_like_choice_legend(colh) and not _looks_like_definition_sentence(colh):
                    # Avoid attaching timepoint-like values as headers for right-side value cells.
                    if not (_is_timepointish_text(colh) and cx >= left_margin_x + 190.0):
                        return _norm_text(f"{base} - {colh}")
            return base

    # Fallback: closest reasonable line above.
    best = None
    best_score = None
    for ln in text_lines:
        ...
        score += -12.0 if _promptish_text(t) else 0.0
        score += 12.0 if (_valueish_text(t) and x >= left_margin_x + 170.0) else 0.0
        if best_score is None or score < best_score:
            best_score = score
            best = ln
    ...
    if best is None:
        return ""
    return _wrap_join(best, text_lines, small_sz, left_margin_x, x_max=cx + 60.0)


def _infer_field_label_for_type_line(lines, type_ln, small_sz: float, text_lines, left_margin_x: float):
    ...
    for ln in text_lines:
        ...
        if y < cy - 50.0 or y > cy + 280.0:
            continue
        if x < left_margin_x - 12.0 or x > left_margin_x + 620.0:
            continue
        if _looks_like_choice_legend(t) or _looks_like_definition_sentence(t):
            continue
        if _is_optionish_shape(t, x, left_margin_x):
    ...
    if not cands:
        # If only TYPE lines appear and no nearby text, don't hallucinate a field label.
        return ""

    cands.sort(key=lambda z: z[0])
    best_ln = cands[0][1]
    return _wrap_join(best_ln, text_lines, small_sz, left_margin_x, x_max=cx + 80.0)
...
def extract(pages):
    ...
    for page_index, lines in pages:
        ...
        text_lines = [ln for ln in lines2 if _text_candidate(ln, small_sz)]
        if not text_lines:
            continue

        left_margin = min((_f(getattr(ln, "x0", 0.0)) for ln in text_lines), default=0.0)
        header_lines = [ln for ln in lines2 if _header_like(ln, small_sz)]
        two_col_split = _two_col_split_x(text_lines)

        recs = []
        for mk, kind in markers:
            if kind == "type":
                base = _infer_field_label_for_type_line(lines2, mk, small_sz, text_lines, left_margin)
                if not base:
                    base = _infer_field_label(lines2, mk, small_sz, text_lines, left_margin, header_lines, two_col_split)
            else:
                base = _infer_field_label(lines2, mk, small_sz, text_lines, left_margin, header_lines, two_col_split)
            ...
            if _looks_like_choice_legend(base) or _looks_like_definition_sentence(base):
                continue
            # Avoid emitting pure timepoint-like values as field labels.
            if _is_timepointish_text(base) and (" - " not in base) and (len(base) <= 22):
                continue
        ...
        for r in sorted(recs, key=lambda d: (d["y"], d["x"])):
            ...
            if _looks_like_choice_legend(field_name) or _looks_like_definition_sentence(field_name):
                continue
            if _is_timepointish_text(field_name) and (" - " not in field_name) and (len(field_name) <= 22):
                continue
