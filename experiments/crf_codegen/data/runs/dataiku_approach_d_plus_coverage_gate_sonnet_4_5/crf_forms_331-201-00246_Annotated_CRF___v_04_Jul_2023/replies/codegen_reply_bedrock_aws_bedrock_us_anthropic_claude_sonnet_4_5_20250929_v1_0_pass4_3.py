...
1. **Version history tables (page 11, 13)**: The current detection is failing because it's looking at the wrong structural pattern. Version history pages have a clear table structure with column headers (Version, Date, Changed By, Details) followed by data rows. I'll detect these by identifying pages where multiple lines share the same x-coordinates (columnar layout) and contain version numbers/dates in a tabular pattern, then skip the entire page.

2. **Copyright/attribution pages (cluster 0 - pages 480, 776)**: These are sparse pages with just copyright text and attribution. The current filter works but needs refinement - these pages have very few lines (< 15 substantive lines) and contain the copyright notice. I'll strengthen this detection.

3. **Simple instruction pages (cluster 2 - pages 955, 958)**: These have a colored title and a single instruction line in brackets. They're legitimate non-field pages. I'll detect pages with only a colored title and bracketed instruction text (< 5 substantive lines total).

4. **Concatenated field names (page 42)**: "Bilateral Oophorectomy Bilateral Tubal Ligation" is being extracted as one field when they should be separate or skipped entirely. This happens when multiple checkbox options are on the same line. I'll detect this pattern by looking for multiple capitalized phrases without punctuation separation and either split them or skip them based on context.

5. **Missing fields (page 42, 46)**: "Hysterectomy" and the long question about reproductive status are not being extracted. The first might be too short or in an unexpected position; the second is likely too long (> 150 chars). I'll relax the length constraint for question-like text (containing "?" or starting with interrogative words) and lower the minimum length threshold for bold text.

6. **Wrong extractions (page 46)**: "Page?" is being extracted when it's part of a longer question. This suggests the multi-line continuation logic is breaking. I'll improve the line continuation detection to better handle wrapped questions.

7. **Coverage gaps (clusters 0, 2, 3)**: 64% of content pages are uncovered. I need to be less aggressive about skipping pages and more inclusive about what constitutes a field, while still filtering out non-fields structurally.

The approach: carry forward form titles across pages; detect version tables by columnar structure with version/date patterns; detect copyright pages by sparsity + copyright text; detect instruction-only pages by colored title + single bracketed line; improve field extraction to handle short bold fields, long questions, and better line continuation; split or skip concatenated checkbox options by detecting multiple capitalized phrases.
