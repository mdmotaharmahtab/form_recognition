...
This revision addresses the false positives while maintaining existing coverage:

1. **Form title handling**: Continue carrying forward the current form title across pages. Titles are large (≥14pt), blue, near the top. Skip TOC pages (many numbered items like "3.120.").

2. **Separate fields from non-fields structurally**:
   - **Fields**: Left-aligned (x < 150), black text, 7-13pt, not part of vertical option lists
   - **Option lists**: Right-aligned (x > 300) OR vertical clusters of 5+ items with regular spacing and similar x-position
   - **Instructional text**: Very long text (>200 chars) or multi-sentence blocks, especially those starting with question words or imperative verbs
   - **Table rows**: "Row N" patterns
   - **Conditional prompts**: Text containing "If Yes then please fill" patterns - these are navigation instructions, not fields

3. **New detection for problematic patterns**:
   - Detect "Select all that apply" as an instruction header (often precedes option lists)
   - Detect long instructional sentences that contain semicolons and describe procedures ("must ask about all types", "with the act, then it can be considered")
   - Detect question-style instructions that are data collection guidance rather than field labels ("Have you done anything to harm yourself?")
   - Detect conditional navigation text ("If Yes then please fill in AE page")

4. **Coverage of all layouts**:
   - Cluster 0-2, 7-8: Already covered (field extraction working)
   - Cluster 1: Some pages have option lists that need better filtering
   - Cluster 3, 4, 6: Pure option reference pages (right-aligned lists) - correctly skip
   - Cluster 5: TOC pages - correctly skip

5. **Structural rules**: Enhance the instructional text detector to catch procedural guidance and conditional prompts. Improve vertical option list detection to catch lists that start with "Select all that apply".
...
def extract_fields_from_page(lines: List, page_num: int) -> List[Dict]:
    ...
    while i < len(lines):
        ...
        if line.x0 < 150 and not line.non_black:
            ...
            # Exclude conditional navigation prompts
            if is_conditional_prompt(field_text):
                i = j
                continue
...
def is_conditional_prompt(text: str) -> bool:
    """Detect conditional navigation prompts like 'If Yes then please fill in AE page'."""
    # These are instructions to navigate to other forms, not field labels
...
def is_instructional_text(text: str) -> bool:
    ...
    # Detect question-style instructions (data collection guidance, not field labels)
    # These are questions the interviewer asks, not field names
