...
The program already handles most pages well (96% content coverage). The two main issues are:

1. **Missing fields on pages 127 and 130**: These are C-SSRS questionnaire pages where the questions "Have you made a suicide attempt?" and "Has there been a time when you started to do something to try to end your life but you stopped yourself before you actually did anything?" are being filtered out by the `is_instructional_text()` function. These are legitimate field labels (questions), not instructions. The function is too aggressive in filtering long questions. We need to allow questions that are direct field prompts, even if they're longer than 60 characters.

2. **Cluster 3, 5 pages (0% coverage)**: These are answer option reference pages (lists of medication forms, routes, etc.) that should be skipped - zero coverage is correct.

3. **Cluster 1 pages (558, 866)**: These show vertical lists of chemistry assay options. The current code correctly identifies these as option lists and skips them, which is appropriate.

The fix is to refine `is_instructional_text()` to distinguish between:
- **Instructional guidance** (procedural text, multi-part directions) → filter out
- **Direct field questions** (even if long) → keep as fields

Key indicators that a long question is a field label:
- Starts with question words (Have, Has, Did, Were, Was, etc.) followed by "you" or "there"
- Is a single, complete question (one question mark)
- Doesn't contain instructional keywords like "Ask questions", "If both are", "must ask about"

We'll adjust the logic to allow direct questions while still filtering procedural instructions.
...
def is_instructional_text(text: str) -> bool:
    ...
    # First, check if this is a direct field question (should be kept)
    # Direct questions start with question words and are single, complete questions
    if is_direct_field_question(text):
        return False
...
def is_direct_field_question(text: str) -> bool:
    """
    Determine if text is a direct field question (should be kept as a field).
    Direct questions are single, complete questions that prompt for data entry.
    """
    ...
    # Check if it starts with common question patterns for field prompts
    question_starters = [
        r'^Have you\b',
        r'^Has there been\b',
        r'^Did you\b',
    ...
    for pattern in question_starters:
        if re.search(pattern, text, re.IGNORECASE):
            # It's a direct question - now check it's not instructional
            # Instructional questions contain phrases like "Ask", "If", "When"
            if not re.search(r'\b(Ask|If both|If the answer|When person|must ask)\b', text, re.IGNORECASE):
                return True
