...
This revision addresses the specific false positives and missed fields while preserving existing coverage:

1. **Form title tracking**: Continue carrying forward the current form title from large blue text near the top of each page. When no new title appears, the previous form_name persists across pages.

2. **False positive elimination**:
   - "Abnormal, Not" appears to be a checkbox option or table cell value, not a field label. Filter out short phrases (< 15 chars) that appear in right-aligned or center positions (x > 300) and lack question markers.
   - "Sertraline PK", "Brexpiprazole/Sertraline PK", "Propranolol PK", "Prazosin PK" are likely column headers or row labels in a PK table. Exclude text matching the pattern of drug names followed by "PK" when they appear in tabular contexts (multiple similar items at similar y-coordinates, x > 200).

3. **Missing field recovery**:
   - "PGx" on page 1070 was missed. This is likely a short but valid field label. Relax the minimum length requirement for left-aligned labels (x < 200) to allow 2-3 character fields when they have appropriate styling (size 9-12pt, not bold/huge).

4. **Structural separation**:
   - Data-entry field labels: left-aligned (x < 250), medium size (8.5-12pt), complete sentences or clear labels
   - Answer options: right-aligned (x > 400), short text, often in columns
   - Table headers in data regions: when appearing as repeating patterns with x > 200 and similar y-coordinates
   - Continuation lines: join text at similar x-position and close y-distance

5. **Coverage**: Process all pages, extract from header-only tables (cluster 2 pattern) and question-based forms, never skip pages based on density.
