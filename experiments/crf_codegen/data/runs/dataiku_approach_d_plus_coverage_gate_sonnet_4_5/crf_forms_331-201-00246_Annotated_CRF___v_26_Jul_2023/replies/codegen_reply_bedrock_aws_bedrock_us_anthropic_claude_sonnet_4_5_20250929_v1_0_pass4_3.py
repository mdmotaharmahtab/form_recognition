...
This revision addresses the core problems:

1. **Form title tracking**: The form title is large (≥15pt) colored text (#004c99). Once found, it persists across subsequent pages until a new title appears. This handles pages where the title is absent or positioned differently.

2. **Structural field identification**: Data-entry fields are black text in the 8.5-11pt range, positioned in the main content area (y > 150, typically left-aligned around x=60-70). They are NOT:
   - Table column headers (single short words at y < 150)
   - Table row data (aligned in columns at specific x positions > 200)
   - Version numbers or dates (numeric patterns like "0.1", "0.1.1", dates)
   - Author names or short phrases in change logs (detected by position in tabular layouts)
   - Answer options (bullets, "Yes/No", etc.)
   - Copyright/definition text (detected by y position and content patterns)

3. **Change History table detection**: Pages 11-12 contain a change log table with version numbers, descriptions, and author names arranged in columns. These are NOT fields. Detected by: multiple short numeric/date-like entries, tabular x-position alignment (columns at x~60, x~200, x~400), and presence of version patterns.

4. **Coverage of all clusters**:
   - Cluster 0 (copyright/definition pages): Correctly skipped - no fields present
   - Cluster 1 (table header pages): Extract fields if present below headers (y > 150)
   - Cluster 2 (repeatable row instruction pages): Correctly skipped - no fields present
   - Clusters 3-4: Continue extracting as before

5. **Structural discrimination**: No hardcoded text blocklists. Instead, use:
   - Y-position thresholds to separate headers from content
   - X-position clustering to identify table columns vs. field labels
   - Size and color to identify titles vs. fields
   - Pattern matching for version numbers/dates (structural, not literal strings)
