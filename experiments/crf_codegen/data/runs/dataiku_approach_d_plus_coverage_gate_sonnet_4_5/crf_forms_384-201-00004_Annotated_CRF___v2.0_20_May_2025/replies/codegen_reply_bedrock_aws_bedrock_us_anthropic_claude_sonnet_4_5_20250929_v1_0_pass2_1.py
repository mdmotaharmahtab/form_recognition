...
The document is a clinical trial CRF with annotated field specifications. Fields appear with human-readable labels on the left side (around x=46.5) and technical metadata on the right side (around x=453.5). Form/section titles appear as bold colored headers (typically #31708f) at larger font sizes (around 10.5pt) with "Origin: CRF" metadata nearby. I will extract form_name from these section headers and carry them forward across subsequent pages until a new section header appears. Field labels are identified by their position on the left side, typically at y-positions that align with answer options or input placeholders (shown in brackets like [____] or radio buttons shown as "O"). I will distinguish fields from answer options by recognizing that options appear indented below their field label and are prefixed with "O" for radio buttons. Technical annotations in brackets like [SAS Field Name:...] or [PETEST_...] are landmarks but not field names themselves. The actual field label is the human-readable text preceding these technical markers. Reference tables (like the coagulation panel or code lists) contain column headers and data rows but no entry fields, so I will skip pages that show only tabular reference data without input placeholders. Pages showing "Electronic Record and Signature Disclosure" or "aCRF Approval Form" are administrative cover pages with no data-entry fields. I will process all pages sequentially, maintaining state for the current form_name, and extract every field label that has an associated input mechanism (brackets, radio buttons, or checkboxes).
