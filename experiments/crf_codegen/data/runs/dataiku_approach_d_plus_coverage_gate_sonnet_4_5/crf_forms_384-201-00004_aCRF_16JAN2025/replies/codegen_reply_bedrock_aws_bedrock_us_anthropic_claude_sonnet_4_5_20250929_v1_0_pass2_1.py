...
I will extract data-entry fields from this clinical CRF document by identifying field labels through structural patterns. Form names appear as large colored headers (size ~12pt, white text on colored background) at the top of field-bearing pages, and I will carry forward the most recent form name across subsequent pages until a new one appears. Field labels are identified by their position on the left side of the page (x < ~250), medium font size (~7-8pt), and are followed by input indicators like brackets, underscores, or radio buttons. I distinguish fields from answer options by checking that options appear inline or immediately below their field label at similar or slightly indented x-positions, while true fields have distinct labels. Machine codes in square brackets (e.g., [PCDTC], [SAS Field Name: ...]) and technical annotations in small fonts (< 6pt) on the right side (x > ~450) are landmarks but not extracted as field names. Multi-line field labels wrapping across consecutive lines at the same x-position are joined into single field names. Reference tables and enumeration lists (like the Study Events table on page 2 or code lists on page 138) are identified by their tabular structure with column headers and are excluded since they lack per-row data-entry cells. Pages without recognizable form headers use the last seen form name to ensure continuity across multi-page forms. I process every page and extract all qualifying field labels, using geometry and structural patterns rather than specific text content to generalize across the entire 206-page document.
