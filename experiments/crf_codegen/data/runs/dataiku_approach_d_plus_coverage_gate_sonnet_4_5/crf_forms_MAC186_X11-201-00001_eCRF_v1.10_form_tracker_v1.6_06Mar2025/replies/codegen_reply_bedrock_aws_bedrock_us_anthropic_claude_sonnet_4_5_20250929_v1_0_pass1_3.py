...
1. **Form title handling**: Continue carrying forward the current form name across pages. The visit separator pages (cluster 2) showing only "Visit:" and "Day N" are correctly identified and skipped - they contain no data-entry fields, so zero coverage is appropriate.

2. **Filtering out answer options**: The false positives ("Yes No", "American Indian or Alaska Native...", etc.) are answer options or checkbox lists, not field labels. These appear as non-bold text or as long concatenated strings of options. I'll strengthen the filter to exclude:
   - Non-bold text that looks like answer choices (short phrases like "Yes No")
   - Very long text strings that are clearly concatenated option lists
   - Text that appears immediately after a field label at a slightly indented position (typical of radio button/checkbox options)

3. **Filtering out "Reason Not Performed"**: This appears on 70%+ of pages, indicating it's likely a recurring template element (perhaps a standard field on many forms, or page furniture). I'll examine its structural characteristics - if it appears at consistent positions across diverse page types, it may be furniture. However, if it's a legitimate field appearing on many different forms, it should be kept. I'll add logic to detect if text appears in a footer-like position or with furniture-like consistency.

4. **Multi-line label handling**: Keep the existing logic that concatenates continuation lines for wrapped field labels.

5. **Coverage**: The program already covers clusters 0 and 1 (100%). Cluster 2 pages are visit separators with no fields, so 0% coverage is correct.

The key structural rules:
- Field labels: bold, moderate size (6-11pt), left-aligned (x < 450)
- Answer options: non-bold OR very long concatenated text OR positioned as indented list items
- Furniture: text at extreme y positions (header/footer zones) or recurring at identical positions across unrelated forms
