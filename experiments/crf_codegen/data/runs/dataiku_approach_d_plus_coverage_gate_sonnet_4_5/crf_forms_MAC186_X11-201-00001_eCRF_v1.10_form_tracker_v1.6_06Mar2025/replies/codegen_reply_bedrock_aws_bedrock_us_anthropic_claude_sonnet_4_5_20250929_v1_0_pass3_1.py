...
The document is a clinical CRF with multiple form sections, each containing data-entry fields. Form titles appear in two locations: a small bold header near the top (y≈48, size≈7.8) and a larger title below (y≈74, size≈17.4). I will use the larger, more prominent title as the primary form_name and carry it forward across continuation pages. Field labels are identified by their position and context: they appear as questions or parameter names, typically with bold styling and bracketed numbers nearby (field codes). I will distinguish fields from non-fields structurally: answer options (Yes/No pairs) sit immediately below their field with consistent x-positions; table rows in reference lists (like medication tables in family D) lack individual entry cells and are not fields; "Variable details" pages are metadata documentation, not data-entry forms. Multi-line labels will be joined by detecting continuation lines at similar x-positions. When a page lacks a form title, I will carry forward the most recent title encountered. Pages with very small fonts (size<6) in dense tabular layouts (family E with Yes/No columns) are likely data tables rather than field-entry forms, but I will extract time-point labels as fields if they represent distinct data collection points. I will skip "Variable details" pages (metadata tables with "Export Name" columns) as they document the CRF structure rather than collect data. For all other pages, I extract every label that represents a question or parameter to be filled in, using structural cues (bold text, bracketed codes, position relative to entry areas) rather than specific wording.
