...
1. "Answer(s):" is being extracted as a field but it's actually a section header/label marker, not a data-entry field
2. "Staff Initials:" and "Comment:" are being skipped but they ARE data-entry fields
3. Need to extract more fields that are currently being missed

The issue is my logic is backwards - I'm skipping "Staff Initials:" and "Comment:" when these are actual fields, but extracting "Answer(s):" which is just a section marker.

Let me analyze the structure:
- Field labels are bold at x ≈ 167.7
- "Answer(s):" appears to be a section header that precedes answer options, not a field itself
- "Staff Initials:" and "Comment:" are actual data-entry fields that should be extracted
    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            if line.bold and 160 < line.x0 < 175:
                ...
                # Skip non-field labels that are section headers
                if text in ["Activity", "Timepoint"]:
                    i += 1
                    continue
                
                # Skip "Answer(s):" - it's a section header, not a field
                if text == "Answer(s):":
                    i += 1
                    continue
                ...
                # Skip "Answer(s):" if it got through somehow
                if field_name == "Answer(s):":
                    i = j if j > i + 1 else i + 1
                    continue
