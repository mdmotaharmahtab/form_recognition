    ...
    for page_idx, lines in pages:
        ...
        for line in lines:
            ...
            if (line.size >= 15.0 and line.non_black and 
                ('#004c99' in str(line.__dict__) or '#2477cc' in str(line.__dict__)) and
        ...
        # Check if this is a blank/title-only page (cluster 3, 4, 5, 6)
        ...
        # Skip pages with only title and no content (clusters 3, 4, 5, 6)
            ...
            if not line.non_black and 8.0 <= line.size <= 11.0:
                ...
                while j < len(lines):
                    ...
                    if (not next_line.non_black and
                        ...
                        abs(next_line.size - line.size) < 2.5):
                ...
                if ('©' in field_text or 'copyright' in field_text.lower() or
                    field_text.startswith('**') or field_text.startswith('Definitions of')):
                    i = j
                    continue
                
                # Skip if it's a row label (structural, not a field)
                if re.match(r'^Row \d+$', field_text):
                    i = j
                    continue
                
                # Detect if this is a list item without context (just an option name)
                ...
                is_bare_list_item = (len(field_text.split()) <= 3 and 
                                    ...
                                    not any(kw in field_text for kw in ['Date', 'Time', 'Number', 'Was', 'Is', 'Did', 'Does', 'Has', 'Have', 'Will']))
                ...
                is_complete_field = (len(field_text) >= 15 or 
                                   ...
                                   any(kw in field_text for kw in ['Date', 'Time', 'Number', 'Was', 'Is', 'Did', 'Does', 'Has', 'Have', 'Will', 'Subject', 'Patient', 'Clinician']))
