    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            if not line.non_black and 7.5 <= line.size <= 11.5:
                ...
                # Detect C-SSRS instruction blocks that are too detailed
                ...
                # OR contains very long explanatory text about suicide attempts/behavior
                    ...
                    (re.match(r'^Row \d+', field_text) and
                     ...
                     any(phrase in field_text for phrase in
                         ['must ask about', 'does not have to be', 
                          ...
                          'Examples are similar', 'instead of being stopped'])) or
                    # Also catch long explanatory text that starts with "Row N" pattern
                    (field_text.startswith('Row ') and 
                     len(field_text) > 250 and
                     field_text.count('.') >= 2 and
                     any(phrase in field_text for phrase in
                         ['Select all that apply', 'so long as these are separate events',
                          'A potentially self-injurious act', 'Intent does not have to be',
                          'can be considered an actual', 'There does not have to be any injury']))
                )
                ...
                # Detect long explanatory text about self-injurious behavior (not a field label)
                # These are multi-sentence explanations that describe concepts
                is_behavior_explanation = (
                    len(field_text) > 200 and
                    field_text.count('.') >= 2 and
                    any(phrase in field_text for phrase in
                        ['When person begins to take steps', 'stops themselves before they actually',
                         'stops him/herself, instead of being stopped',
