...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            if line.non_black and line.text.strip().startswith('['):
                i += 1
                continue
            ...
            if not line.non_black and line.size >= 9.0 and line.size <= 12.0:
                ...
                # Skip answer option patterns (single words that are likely options)
                # But be careful not to skip real field labels
                if text in ['YES', 'NO', 'Collected', 'Not', 'Scan', 'Predose', 'Postdose']:
                    i += 1
                    continue
                
                # Skip lines that look like enumeration values (in parentheses)
                ...
                if len(text) > 3 and not text.startswith('['):
                    ...
                    while j < len(lines):
                        ...
                        if (not next_line.non_black and 
                            ...
                            # Stop if we hit a new field marker
                            if next_text.startswith('Row ') or next_text in ['YES', 'NO']:
                                break
