...
def extract_cssrs_intensity_since_last(lines, page_num, records):
    ...
    for i, line in enumerate(lines):
        ...
        if line.bold and not line.non_black and line.x0 < 100:
            ...
            # Skip standalone structural words that are not field labels
            # Field labels are typically longer phrases or questions
            if len(text) < 10 and '?' not in text:
                continue
            
            # Valid field label - collect continuation lines
            field_text = text
            j = i + 1
            while j < len(lines):
                next_line = lines[j]
            ...
            # Filter out partial fragments that got split across lines incorrectly
