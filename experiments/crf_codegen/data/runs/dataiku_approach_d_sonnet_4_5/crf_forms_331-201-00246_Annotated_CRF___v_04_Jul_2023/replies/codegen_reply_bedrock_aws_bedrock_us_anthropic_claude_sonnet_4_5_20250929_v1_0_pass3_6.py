...
def identify_layout(lines):
    ...
    has_since_last = any(line.text.strip() == "Since Last Visit" and line.y0 < 200 for line in lines)
    if has_intensity and has_since_last:
        ...
        for line in lines:
            if (line.y0 > 200 and line.bold and not line.non_black and 
...
def extract_cssrs_intensity_since_last(lines, page_num, records):
    ...
    while i < len(lines):
        ...
        # Skip header area (top ~200 pixels to be safe)
        if line.y0 < 200:
            i += 1
            continue
