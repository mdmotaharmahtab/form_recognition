...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        has_changed_by_column = False
        has_details_column = False
        
        for line in sorted_lines[:30]:  # Check first 30 lines
            text = line.text.strip()
            if text.lower() == 'version':
                has_version_column = True
            if text.lower() == 'changed by':
                has_changed_by_column = True
            if text.lower() == 'details':
                has_details_column = True
            if re.match(r'^\d{1,2}-[A-Za-z]{3}-\d{4}', text):
                change_history_indicators += 1
        
        # If we have version table structure, it's a change history page
        if has_version_column and has_changed_by_column and has_details_column:
            is_change_history = True
        elif change_history_indicators >= 3:
            is_change_history = True
        ...
        for line in sorted_lines:
            ...
            if 'columbia' in text.lower() and 'suicide' in text.lower():
                copyright_lines += 1
            if 'psychiatric institute' in text.lower():
                copyright_lines += 1
        
        is_copyright_page = copyright_lines >= 2
        
        # Skip copyright pages entirely
        if is_copyright_page:
            continue
        
        # Detect if this is a simple repeatable row instruction page
        # These have a form title and just "(Repeatable row added with Add Row button)"
        non_furniture_lines = []
        for line in sorted_lines:
            text = line.text.strip()
            # Skip page numbers, empty lines, and the repeatable instruction
            if (text and 
                not re.match(r'^Page \d+', text, re.IGNORECASE) and
                not (text.startswith('(') and text.endswith(')') and 'row' in text.lower())):
                non_furniture_lines.append(line)
        
        # If we only have the form title (colored, large) and nothing else, skip
        if len(non_furniture_lines) <= 1:
            # Check if the one line is just the form title
            if len(non_furniture_lines) == 1:
                line = non_furniture_lines[0]
                if line.size >= 14.0 and line.non_black:
                    continue
            elif len(non_furniture_lines) == 0:
                continue
        ...
        while i < len(sorted_lines):
            ...
            # Skip "Changed By" column headers and similar
            if text.lower() in ['changed by', 'change description', 'version', 'details', 'date']:
                i += 1
                continue
            ...
            technical_keywords = ['deploy', 'uat', 'integration', 'feedback sheet', 
                                 ...
                                 'fixes for', 'implemented', 'applied changes', 'release',
                                 'poe updates', 'cr ', 'added new']
            ...
            # Skip checkbox/radio button labels that are just status indicators
            # These are typically short, bold, and positioned in a specific column
            # Pattern: short bold text in left column that's a status/category
            if line.bold and len(text) <= 30:
                # Check if it's in a left column (x0 < 30% of page width)
                if line.x0 < max_x * 0.3:
                    # Check if it looks like a checkbox option (not a question)
                    # These don't end with ? or : and are typically capitalized phrases
                    if not text.endswith('?') and not text.endswith(':'):
                        # Check if there are similar items nearby (suggesting a list of options)
                        nearby_similar = 0
                        for j in range(max(0, i-3), min(len(sorted_lines), i+4)):
                            if j != i:
                                other = sorted_lines[j]
                                if (abs(other.y0 - line.y0) < 100 and 
                                    abs(other.x0 - line.x0) < 50 and
                                    other.bold and len(other.text.strip()) <= 30):
                                    nearby_similar += 1
                        
                        # If we found 2+ similar items nearby, this is likely a checkbox list
                        if nearby_similar >= 2:
                            i += 1
                            continue
            ...
            # Pattern 3: Single-word labels with field-like keywords
                ...
                field_keywords = ['date', 'time', 'subject', 'patient', 'status', 
                                 ...
                                 'device', 'ligation', 'pills', 'depot', 'vasectomy']
