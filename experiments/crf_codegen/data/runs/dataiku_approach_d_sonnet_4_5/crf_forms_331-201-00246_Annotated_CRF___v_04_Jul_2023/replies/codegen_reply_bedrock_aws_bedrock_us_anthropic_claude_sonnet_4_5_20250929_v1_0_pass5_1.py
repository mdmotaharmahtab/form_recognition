```python
# LAYOUT OBSERVATION AND STRATEGY:
# - Family A (page 1): Table of contents with section numbers and titles in blue
# - Family B (pages 2+): Continuation of TOC with numbered section titles
# - Family C (page 168): Eligibility criteria checklist with codes (INCL/EXCL)
# - Title-context pages show actual CRF forms with field labels, technical codes in red/gray
# - Fields appear as questions/labels in black, with technical codes [VARNAME] in red below
# - Form titles are in large blue font (sz ~16.5, color #004c99)
# - Need to extract field labels (black text) while ignoring technical codes, answer options, and table headers
...
def extract_fields_from_page(lines, form_name: str, page_num: int) -> List[Dict]:
    ...
    while i < len(lines):
        ...
        if (text.startswith('Definitions of') or 
            text.startswith('For reprints') or
            text.startswith('If the subject did not meet')):
            i += 1
            continue
