...
1. **Fix false positives** (page 44, 557): These are instructional text or notes, not field labels. They appear to be continuation lines that shouldn't be included.
2. **Fix missing fields** (page 537): Time fields that should be extracted but aren't.
3. **Handle new layouts** (clusters 2, 1): Table-based forms with "Criteria"/"Met/Not Met" columns and lists of items.
4. **Remove hardcoded string filters** and use structural rules instead.

Let me analyze the sample pages:
- **Cluster 0** (pages 87, 235): Standard forms with field labels, already mostly working
- **Cluster 1** (pages 216, 308): Lists of items (lab tests, codes) - these ARE field labels
- **Cluster 2** (pages 149, 265): Table format with criteria/questions in left column, answer options in right
