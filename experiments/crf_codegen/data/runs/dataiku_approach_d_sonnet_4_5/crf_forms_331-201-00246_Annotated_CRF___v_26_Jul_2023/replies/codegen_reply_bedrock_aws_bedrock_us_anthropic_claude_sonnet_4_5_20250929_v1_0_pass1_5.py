...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        criteria_column_x = None
        
        for line in lines:
            text = line.text.strip()
            # Look for table headers like "Criteria", "Met/Not Met", "Since Last Visit", "Suicidal Behaviour"
            if line.y0 < 150 and line.size >= 9.0:
                if text in ['Criteria', 'Met/Not Met', 'Since Last Visit', 'Suicidal Behaviour']:
                    ...
                    # Track criteria column position
                    if text in ['Criteria', 'Suicidal Behaviour']:
                        criteria_column_x = line.x0
        ...
        while i < len(lines):
            ...
            if has_table_layout:
                ...
                if not line.non_black and line.x0 < 500 and 8.0 <= line.size <= 11.0:
                ...
                while j < len(lines):
                    ...
                    if not next_line.non_black and 8.0 <= next_line.size <= 11.0:
                        ...
                        if '- mark the selected' in cont_text.lower():
                            break
                        # Stop on "please add to" instructions
                        if cont_text.lower().startswith('if yes, please add to'):
                            break
                        
                        # Check horizontal alignment
                        # For table layout, continuation must be in same column (x0 < 500)
                        if has_table_layout and next_line.x0 >= 500:
                            break
