...
def extract(pages):
    ...
    2. For cluster 0: Extract field labels from left column (gray text near red codes)
    3. For cluster 1: Extract bold black questions/labels from left column
    4. For cluster 2: Extract any field labels present
    5. Skip column headers (top of page), answer options (right column), technical annotations (red), and page furniture
    ...
    for page_idx, lines in pages:
        ...
        if is_cluster_0:
            ...
            # Field labels are gray text in left column (x < 240) adjacent to red codes
            ...
            # Look for gray text labels near these positions
            for line in lines:
                text = line.text.strip()
                ...
                if line.x0 < 240 and line.size >= 8.5 and line.size <= 11:
                    ...
                    if is_gray:
                        ...
                        if has_nearby_code and re.search(r'[a-zA-Z]{2,}', text):
