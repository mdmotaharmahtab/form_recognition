...
def extract(pages):
    ...
    - Cluster 0 (~88 pages): Multi-column tables with column headers (Region/Body System, Result, etc.)
      ...
      **Column headers themselves ARE the field labels** - they define the data entry columns.
    ...
    2. For cluster 0: Extract column headers at y ~124 as field labels (these ARE the fields)
    ...
    for page_idx, lines in pages:
        ...
        # Cluster 0: Multi-column table layout with red technical codes and column headers
        # Has headers like "Region/Body System", "Result", "Abnormal Findings", "Sample", "Timepoint"
        # Key: has many gray text lines AND column headers at y ~124
        # The column headers ARE the field labels
        cluster_0_headers = ['Region/Body System', 'Result', 'Abnormal Findings', 
                             'Clinically Significant', 'Sample', 'Timepoint', 
                             'Sample Status', 'Time of', 'Barcode', 'Test']
        has_cluster_0_headers = any(any(h in ht for h in cluster_0_headers) for ht in header_texts)
        is_cluster_0 = len(red_codes) > 10 and has_cluster_0_headers
        ...
        # Headers like "Suicidal Behaviour", "Since Last Visit"
        is_cluster_2 = (len(lines) < 10 and len(red_codes) > 0 and 
                        any(l.y0 >= 120 and l.y0 <= 170 for l in top_headers))
        ...
        if is_cluster_2:
            ...
            for line in lines:
                ...
                # Look for headers at y ~124 (within range 120-170 to be more inclusive)
                if line.y0 >= 120 and line.y0 <= 170 and line.size >= 10 and line.size <= 11:
            ...
            # Cluster 0: Extract column headers at y ~124 as field labels
            # The column headers ARE the actual field labels for data entry
            # Examples: "Region/Body System", "Result", "Abnormal Findings", "Clinically Significant"
            #           "Sample", "Timepoint", "Sample Status", "Time of Collection", "Barcode Number"
            ...
            for line in lines:
                ...
                # Extract column headers at y ~124 (range 120-170)
                # These are black text, size ~10.5, positioned at top of page
                if line.y0 >= 120 and line.y0 <= 170 and line.size >= 10 and line.size <= 11:
                    if not line.non_black:  # Black text only
                        if re.search(r'[a-zA-Z]{3,}', text):
