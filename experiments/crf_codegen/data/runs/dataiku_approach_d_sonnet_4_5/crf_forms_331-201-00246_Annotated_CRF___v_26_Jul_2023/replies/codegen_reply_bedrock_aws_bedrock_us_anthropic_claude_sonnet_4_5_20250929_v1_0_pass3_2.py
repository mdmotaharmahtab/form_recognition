...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        # Layout C: Intensity rating pages - have "Intensity of Ideation" header
        is_intensity_page = False
        has_intensity_header = False
        for line in lines:
            if line.y0 < 200 and 'Intensity of Ideation' in line.text:
                has_intensity_header = True
            if line.y0 < 200 and 'Since Last Visit' in line.text:
                is_intensity_page = True
        
        # Skip schedule/TOC pages - they don't have data-entry fields
        if is_schedule_page:
            continue
        
        # Skip intensity header-only pages (they appear to be continuation markers)
        if is_intensity_page and has_intensity_header:
            # Check if page has actual content beyond headers
            content_lines = [l for l in lines if l.y0 > 200 and not l.non_black 
                           and 'Page' not in l.text and len(l.text.strip()) > 5]
        ...
        while i < len(lines):
            ...
            # Skip bold headers at top of page (y < 200, bold, typically column headers)
