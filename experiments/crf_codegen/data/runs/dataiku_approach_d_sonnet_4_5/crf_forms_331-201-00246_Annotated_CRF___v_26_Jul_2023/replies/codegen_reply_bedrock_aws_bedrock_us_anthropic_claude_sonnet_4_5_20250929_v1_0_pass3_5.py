...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        # If both headers present, it's a schedule/TOC page
        if has_visit_num_header and has_page_num_header:
        ...
        # Layout C: Sparse continuation pages with only headers
        # These have "Intensity of Ideation" and "Since Last Visit" headers at top
        # but no actual field content below (only red annotations and page numbers)
        ...
        # If it's a sparse continuation page (both headers present)
        # Count substantial black text lines to determine if there's real content
        if has_intensity_header and has_since_last_visit:
            # Count non-header, non-red, substantial text lines
            content_lines = 0
            for line in lines:
                if (line.y0 > 200 and 
                    not line.non_black and 
                    not re.match(r'^Page \d+ of \d+', line.text) and
                    len(line.text.strip()) > 15 and
                    not re.match(r'^\(?\d+\)', line.text.strip())):
                    content_lines += 1
            
            # If very sparse (only headers and minimal content), skip
            if content_lines < 3:
                continue
        ...
        while i < len(lines):
            ...
            # Skip if this is a bold header at very top of page (y < 150)
            # These are typically column headers or section markers
            if line.y0 < 150 and line.bold:
                i += 1
                continue
            
            # Skip standalone section headers that appear at top (y < 200)
            if line.y0 < 200:
                ...
                # Skip "Visit Label" and "Page Label" headers
                if text in ['Visit Label', 'Page Label', 'Visit Num', 'Page Num', 
