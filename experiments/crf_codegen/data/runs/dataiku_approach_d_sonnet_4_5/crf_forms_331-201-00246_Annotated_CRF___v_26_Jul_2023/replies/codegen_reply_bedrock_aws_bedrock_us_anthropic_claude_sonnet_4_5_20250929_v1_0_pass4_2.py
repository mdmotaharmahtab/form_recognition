...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        # Change history pages: structured table with Version/Date/Changed By/Details
        # Detect by: has "Change History" title AND has dense tabular data (many lines at similar y positions)
        has_change_title = any("Change History" in line.text and line.size >= 15 for line in lines)
        if has_change_title:
            # Count lines in tabular region (y > 200, regular spacing)
            tabular_lines = [l for l in lines if l.y0 > 200 and l.size < 11]
            if len(tabular_lines) > 20:  # Change history has many data rows
                continue
