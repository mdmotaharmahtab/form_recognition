...
1. **Cluster 0 (pages 22, 48)**: These pages contain data-entry fields that my program is missing. The fields have labels at x~46.5 with bracketed codes like `[QSCSS0322A]`, `[CSS0322A]`, `[RR]`, `[EGTXT]`, `[EGDTC]` etc. The issue is that my program looks for bracketed codes AFTER the label, but on these pages the bracketed code appears BELOW the label text (at a different y-coordinate but similar x-coordinate). I need to adjust the look-ahead logic.

2. **Cluster 1 (pages 150, 163)**: These are code list/reference table pages with "Coded" and "Decode" headers. My program correctly skips these - they contain no data-entry fields.

3. **Cluster 2 (pages 32, 96)**: These pages have fields like "Frequency", "Administration start date and time", "Administration ongoing?" with the bracketed codes appearing below. Similar issue to Cluster 0.

The main problem is that my program's field detection logic expects the bracketed code to appear within 5 lines and at a similar x position, but it's not finding them because the y-distance check (`if next_line.y0 - y > 30: break`) is too restrictive, or the bracketed codes are appearing as separate field identifiers below the label.

Let me update the program to better handle these layouts:
