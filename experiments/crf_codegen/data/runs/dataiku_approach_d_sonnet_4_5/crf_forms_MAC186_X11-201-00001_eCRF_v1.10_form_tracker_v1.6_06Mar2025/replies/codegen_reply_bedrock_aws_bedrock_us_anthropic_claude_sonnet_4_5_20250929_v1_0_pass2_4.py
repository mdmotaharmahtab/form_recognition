...
def extract_standard_fields(lines, form_name, page_num):
    ...
    # Find page height for relative positioning
    ...
    while i < len(lines):
        ...
        # Look for bold field labels (not in header area)
        if line.bold and line.y0 > header_threshold:
            text = line.text.strip()
            ...
            # Skip if it's in the far right column (likely answer options or values)
            # Far right is typically x > 70% of page width
            page_width = max([l.x0 for l in lines]) if lines else 600
            if line.x0 > page_width * 0.7:
                i += 1
                continue
            
            # Skip if it's a single word that's a common answer option
            # (positioned in answer column, not left-aligned field label)
            ...
            while j < len(lines):
                ...
                if y_diff < 20 and y_diff > 0 and x_diff < 100:
                    ...
                    # Skip if it's in far right column
                    if next_line.x0 > page_width * 0.7:
                        break
    ...
    for line in lines:
        if line.bold and abs(line.y0 - header_y) < 10 and line.y0 < 120:
            ...
            if field_text and len(field_text) > 1:
                ...
                if field_text in ["Not Done", "Reason Not Done", "Interpretation"]:
                    # These are positioned in data columns, not field labels
                    # Check if they're in the right side of the table
                    if line.x0 > param_x + 200:
                        continue
