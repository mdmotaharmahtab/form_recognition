...
def extract(pages):
    ...
    for page_index, lines in pages:
        ...
        if column_headers:
            ...
            for y_key in sorted(header_rows.keys()):
                ...
                for x, text in headers:
                    # Skip structural markers: "Row N" pattern (leftmost column, x < 15% of page width)
                    if x < 80 and re.match(r'^Row \d+$', text, re.IGNORECASE):
                        continue
                    # Skip repeatable row instructions (parenthetical notes)
                    if text.startswith('(') and text.endswith(')'):
                        continue
                    # Skip position labels in leftmost column (x < 15% of page width)
                    if x < 80:
                        continue
                    # Skip calculated/derived fields (contain "Difference" or "Calculated")
                    if 'Difference' in text or 'Calculated' in text:
                        continue
        ...
        while i < len(lines):
            ...
            # Skip page numbers: small text, numeric only, positioned at extremes (top/bottom 10%)
            if re.match(r'^\d+$', text) and line.size < 9.0:
                i += 1
                continue
            
            # Skip structural markers in leftmost column (x < 15% of page width)
            if line.x0 < 80:
                # Skip "Row N" pattern
                ...
                if re.match(r'^(Standing|Supine|Sitting|Prone)$', text):
                    i += 1
                    continue
                # Skip contact status labels
                if re.match(r'^(Final|First|Second|Third)', text):
                    i += 1
                    continue
            ...
            # Skip answer options positioned to the right (x > 70% of page width)
            if line.x0 > 400 and re.match(r'^(Yes|No)$', text):
                i += 1
                continue
            ...
            # Skip "If Yes/No" prompts when standalone and short
            ...
            if line.size >= 7.0 and line.size <= 11.0:
                ...
                # Skip pure instruction or note text (starts with "Note:" or "Instructions:")
                if re.match(r'^(Note|Instructions):', text):
                    i += 1
                    continue
