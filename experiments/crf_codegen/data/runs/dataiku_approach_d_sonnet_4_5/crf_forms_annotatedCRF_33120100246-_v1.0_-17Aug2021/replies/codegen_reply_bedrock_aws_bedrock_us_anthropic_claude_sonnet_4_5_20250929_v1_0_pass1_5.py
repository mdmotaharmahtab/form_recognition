...
def is_valid_field_label(text, layout_info):
    ...
    # Skip instructional text that starts with "If" or "Please" - these are instructions
    # not field labels. Field labels are typically nouns/questions, not imperatives.
    if re.match(r'^(if|please)\s+', text_stripped.lower()):
        return False
