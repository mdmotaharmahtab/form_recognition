...
**Cluster 0 & similar pages (250, 420)**: These are C-SSRS cover/disclaimer pages - correctly skipped (no fields).

**Cluster 1 pages (252, 422)**: These contain actual C-SSRS question fields! The bold questions like "Have you been thinking about how you might do this?" and "Have you had these thoughts and had some intention of acting on them?" are data-entry fields. My program is incorrectly skipping them because they start with numbered section headers like "3. Active suicidal ideation..." which my filter excludes.

**Cluster 2 pages (290, 465)**: These appear to be technical annotation pages showing field codes and types in red/gray - these are genuinely field-free (just showing metadata).

I need to fix the handling for Cluster 1 pages - they have legitimate question fields that should be extracted.
...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        red_count = sum(1 for line in lines if line.non_black and '[TYPE:' in line.text)
        if red_count > 5:
            continue
        ...
        while i < len(lines):
            ...
            # Bold questions/prompts (including those that start with numbers)
