...
def extract(pages):
    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            # Potential Lethality field label
            if 'Potential Lethality:' in text and line.bold:
                is_field = True
            
            # Long descriptive text that's a field label (not rating anchors)
            # These are at x0 < 100, not bold, size 7-9, black
            if (not line.bold and line.x0 < 100 and 7 <= line.size <= 9 and
                len(text) > 30 and not text.startswith('\\')):
                # Check if it's a substantive description (not a rating anchor)
                # Rating anchors have patterns like "e.g." or multiple semicolons
                semicolon_count = text.count(';')
                if semicolon_count <= 1 and 'e.g.' not in text.lower():
                    # This might be a field description
                    if 'lethality' in text.lower() and 'actual attempt' in text.lower():
                        is_field = True
            ...
            if is_field:
                ...
                # Final validation: must be substantive and not a rating anchor
