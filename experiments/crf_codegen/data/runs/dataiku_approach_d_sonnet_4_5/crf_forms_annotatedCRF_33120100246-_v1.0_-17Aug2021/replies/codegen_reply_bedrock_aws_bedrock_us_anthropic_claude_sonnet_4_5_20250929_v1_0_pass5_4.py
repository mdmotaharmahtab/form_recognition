...
def extract(pages):
    ...
    for page_index, lines in pages:
        ...
        if column_headers:
            ...
            for y_key in sorted(header_rows.keys()):
                ...
                for x, text in headers:
                    # Skip structural markers: "Row N" pattern
                    if re.match(r'^Row \d+$', text, re.IGNORECASE):
                        continue
                    # Skip repeatable row instructions (parenthetical notes)
                    if text.startswith('(') and 'Repeatable' in text:
                        continue
                    # Skip position labels in leftmost column (x < 100)
                    if x < 100 and text in ['Standing', 'Supine', 'Sitting', 'Prone']:
                        continue
        ...
        while i < len(lines):
            ...
            # Skip page numbers: small text, numeric only, positioned at extremes
            ...
            # Skip answer options positioned to the right (x > 400)
            ...
            # Skip position labels in leftmost column (x < 100)
            if line.x0 < 100 and text in ['Standing', 'Supine', 'Sitting', 'Prone']:
                i += 1
                continue
            ...
            # Skip contact status labels in leftmost column (x < 150)
            if line.x0 < 150 and re.match(r'^(Final|First|Second|Third)', text):
                i += 1
                continue
