...
1. **Fix missed fields on pages 77, 90**: Short labels like "PR", "QRS", "QT", "BMI" are being filtered out by my regex `^[A-Z][A-Z0-9_]+$` check
2. **Handle cluster 1 (81 pages, 0% coverage)**: These are reference table pages (like "CM Dose Units", "Dose Route") that I'm currently skipping entirely - but they should be skipped, they contain no data-entry fields
3. **Keep existing coverage**: Don't break the 94% coverage on cluster 0

The main issue is my field validation logic is too aggressive. Short uppercase labels like "PR", "QRS", "QT", "BMI" are legitimate field names, not machine codes. The distinguishing factor: machine codes have underscores or are bracketed technical identifiers, but these short medical abbreviations are actual field labels.
