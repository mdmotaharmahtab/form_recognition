...
# Page 71 fix: these are fields on the Physical Exam page; if they appear as prominent
# colored headers, they must NOT become the form name. Instead, they are fields under
...
def _extract_fields_by_label_colon_pattern(lines, current_form, page_num, seen):
    ...
    Heuristic: left-column label lines ending with ':' or matching known PE fields.
    """
    out = []
    ...
    for ln in left:
        ...
        is_known_pe_field = t.lower() in PE_FIELD_HEADERS
...
def _extract_pe_fields_from_colored_headers(lines, current_form, page_num, seen):
    """
    Page 71 fix: On Physical Exam pages, the fields (Abnormal findings, Body system,
    Examination result, Clinical significance) can appear as prominent colored headers.
    The previous logic mistakenly treated them as form switches. Here we extract them
    as fields when they appear as colored header-like lines.
    """
    out = []
    for ln in lines:
        t = _norm_space(ln.text)
        if _is_noise_line(t):
            continue
        low = t.lower()
        if low not in PE_FIELD_HEADERS:
            continue

        # Must look like a header-ish line (colored and reasonably large), near left.
        if not ln.non_black:
            continue
        if ln.size < 9.0:
            continue
...
def _detect_physical_exam_context(lines) -> bool:
    """
    Detect whether the page is a Physical Exam page even if the banner/header is missed.
    """
    for ln in lines:
        t = _norm_space(ln.text)
        if _is_noise_line(t):
            continue
        if "physical exam" in t.lower() or "physical examination" in t.lower():
            # banner or section title
            if ln.x0 < 220 and ln.y0 < 260:
                return True
    # If multiple PE field headers appear, it's almost certainly the PE page.
    hits = 0
    for ln in lines:
        t = _norm_space(ln.text).lower()
        if t in PE_FIELD_HEADERS:
            hits += 1
    return hits >= 2
...
def extract(pages):
    ...
    for page_idx0, lines in pages:
        ...
        # 2) Additional form-switch detection to fix wrong attribution
        fn2 = _page_has_form_switch_header(lines)
        if fn2:
            # IMPORTANT: Do not allow PE field headers to become the form name.
            # This is the root cause of page 71 wrong-form attribution.
            if fn2.strip().lower() not in PE_FIELD_HEADERS:
                current_form = fn2

        # 2b) If page looks like Physical Exam context, force form to Physical Examination
        # (prevents carry-over from prior form when header extraction fails).
        if _detect_physical_exam_context(lines):
            current_form = "Physical Examination"
        ...
        # 3b) Page 71 fix: extract PE fields from colored headers (even without colons/brackets)
        # Run always; dedupe via `seen`.
        out.extend(_extract_pe_fields_from_colored_headers(lines, current_form, page_num, seen))
