    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            if line.bold and line.text.strip():
                ...
                # Skip lines that are just parenthetical notes or instructions that don't look like fields
                # But keep parenthetical notes that look like field qualifiers
                if text.startswith("(") and text.endswith(")"):
                    # Keep if it looks like a field qualifier (e.g., "(Supine for at least 3 minutes)")
                    if not any(keyword in text.lower() for keyword in ["supine", "collect", "perform", "for at least"]):
                        i += 1
                        continue
                ...
                # Skip if it's just a parenthetical note (but not field qualifiers)
                if full_question.startswith("(") and full_question.endswith(")"):
                    if not any(keyword in full_question.lower() for keyword in ["supine", "collect", "perform", "for at least"]):
                        i += 1
                        continue
                ...
                if (not full_question.endswith("?") and 
                    ...
                    full_question.lower().startswith(("admission", "visit", "smoke since"))):
                    i += 1
                    continue
