...
**Cluster 10 (pages 129, 526)**: These pages contain data-entry fields! The field labels are in gray (#454545) text like "Benzodiazepines", "Cannabinoids", "Cocaine", "Methadone", "Opiates", "Phencyclidine". Each has associated red technical markers [LBTEST3] and [LBORRES3]. The current program skips these because they're in gray, not black.

**Cluster 11 (pages 194, 359)**: These are disclaimer/copyright pages for the C-SSRS form - genuinely field-free, just instructional text.

The program needs to be updated to handle gray field labels that appear before red technical markers.
...
3. Some field labels appear in gray (#454545) text, especially in table-based layouts
4. Technical codes in red (e.g., [VISDAT], [TYPE: ...]) mark field boundaries but are not extracted
5. Answer options (Yes/No/etc.) in gray (#454545) are not fields themselves
6. Multi-column layouts exist; we use x-coordinates to distinguish columns

Strategy: Identify form headers by size/color, then extract black or gray text labels that precede
