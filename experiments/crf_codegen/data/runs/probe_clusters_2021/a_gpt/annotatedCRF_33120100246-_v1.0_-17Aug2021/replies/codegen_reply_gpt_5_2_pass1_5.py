...
_RE_TYPE_PREFIX = re.compile(r"^\[\s*TYPE\s*:", re.IGNORECASE)
...
def _try_join_broken_any_bracket(lines, i, max_join_lines=6):
    """
    Join broken bracket tokens across consecutive colored lines, allowing ':' and spaces.
    Intended for [TYPE: ...] style tokens on pages where var codes are absent.
    Returns (joined_text, j) or (None, i).
    """
    t0 = _norm_text(getattr(lines[i], "text", "") or "")
    if not t0 or not t0.startswith("[") or t0.endswith("]"):
        return (None, i)
    if not getattr(lines[i], "non_black", False):
        return (None, i)
    ...
    parts = [t0]
    j = i
    for _ in range(max_join_lines - 1):
        if j + 1 >= len(lines):
            break
        nxt = lines[j + 1]
        if not getattr(nxt, "non_black", False):
        ...
        size1 = getattr(nxt, "size", None)
        if size0 and size1 and abs(float(size1) - float(size0)) > 1.8:
            break
        ...
        joined = _norm_text(" ".join(parts))
        # Avoid runaway joins that scoop multiple bracket blocks.
        if joined.count("[") > 1:
            return (None, i)
        if len(joined) > 140:
            return (None, i)
...
def _collect_type_markers(lines):
    """
    Used only as fallback on pages with NO var codes.
    Returns list of dicts: {"idx": int, "line": Line, "code_text": str} for [TYPE: ...] markers.
    """
    out = []
    i = 0
    while i < len(lines):
        ...
        if t.startswith("[") and not t.endswith("]"):
            joined, j = _try_join_broken_any_bracket(lines, i, max_join_lines=6)

        cand = joined if joined is not None else t
        cand = _norm_text(cand)
        if cand and _is_bracketed(cand) and _RE_TYPE_PREFIX.match(cand) is not None:
            out.append({"idx": i, "line": l, "code_text": cand})
            i = (j + 1) if joined is not None else (i + 1)
            continue
...
def _label_is_too_optionish(label: str) -> bool:
    """
    Very conservative shape filter to avoid emitting obvious single-token options.
    Do NOT blocklist words; rely on structure + option-group logic for most cases.
    """
    t = _norm_text(label)
    if not t:
        return True
    if "?" in t:
        return False
    # Only treat as "optionish" when extremely short and single-token.
    if t.count(" ") == 0 and len(t) <= 6 and ":" not in t and ";" not in t and "," not in t:
        # Avoid dropping common short acronyms used as real row labels; keep this minimal.
        if len(t) <= 3:
            return True
    return False
...
def _row_has_long_anchor_text(peers, side_wanted):
    for (_, t, _, _, side) in peers:
        if side != side_wanted:
            continue
        tt = _norm_text(t)
        if len(tt) >= 28 and tt.count(" ") >= 4:
            return True
    return False
...
def _inline_left_label(lines, code_idx, code_line, left_margin, header_y_cap):
    ...
    while still avoiding top header furniture.
    """
    cx = float(getattr(code_line, "x0", 0.0) or 0.0)
    cy = float(getattr(code_line, "y0", 0.0) or 0.0)
    ...
    for j in range(code_idx - 1, j0 - 1, -1):
        ...
        # Allow farther-left labels than the computed body margin (tables often start at page edge).
        min_x_ok = max(4.0, left_margin - 200.0)

        # If it's a short, clean label, allow to page edge.
        short_ok = _label_basic_sane(t) and (len(t) <= 24) and (t.count(" ") <= 2)
        if short_ok:
            min_x_ok = 4.0
...
def _label_from_above_band(lines, var_codes, code_idx, code_line, left_margin, header_y_cap, med_sz_all, med_sz_black):
    ...
    while j >= 0:
        ...
        dy = (cy - y0)
        dx = abs(cx - lx)
        score = -1.0 * dy - 0.018 * dx

        question_like = ("?" in t) or _RE_NUM_PREFIX.match(t) is not None or t.endswith(":")
        if "?" in t:
            score += 14.0
        ...
        # Penalize multi-column header rows unless they look like a question/prompt.
        if _header_row_is_multicol(lines, j) and not question_like:
            score -= 35.0

        # Avoid huge headings unless they clearly label a field/code below.
    ...
    x_cap = min(cx + 95.0, left_margin + 720.0)
...
def _classify_group(lines, var_codes, group_info, left_margin, header_y_cap):
    ...
    Decide if a vertical group is:
      ...
      - 'option_list': codes correspond to rating anchors/options; we want ONE question label
    Supports both:
      - option text to the right of code column
      - option text to the left of code column (checkboxes aligned right)
    """
    ...
    right_peer_rows = 0
    left_peer_rows = 0
    left_long_anchor_rows = 0
    right_long_anchor_rows = 0
    any_peer_rows = 0
    ...
    for vi in members:
        ...
        if peers:
            ...
            if any(side == -1 for (_, _, _, _, side) in peers):
                left_peer_rows += 1
            if _row_has_long_anchor_text(peers, side_wanted=-1):
                left_long_anchor_rows += 1
            if _row_has_long_anchor_text(peers, side_wanted=1):
                right_long_anchor_rows += 1
    ...
    left_long_ratio = left_long_anchor_rows / n
    right_long_ratio = right_long_anchor_rows / n

    # Option lists: strong signal if descriptive anchor text exists consistently on one side.
    if right_ratio >= 0.45 and right_ratio >= (left_ratio + 0.15):
        return "option_list"
    if left_long_ratio >= 0.45 and left_long_ratio >= (right_long_ratio + 0.10):
        return "option_list"
    if right_long_ratio >= 0.40 and right_long_ratio >= (left_long_ratio + 0.10):
        return "option_list"

    # Row tables: many distinct per-row labels to the left of the code column.
    if left_labels:
        distinct = len(set(left_labels))
        if distinct >= max(2, int(0.6 * len(left_labels))) and (len(left_labels) / n) >= 0.55:
            return "row_table"

    # Short runs: if there's long anchor evidence, treat as options to avoid emitting anchors.
    if len(members) <= 2 and (left_long_ratio >= 0.35 or right_long_ratio >= 0.35):
        return "option_list"

    # If peers exist but we failed to find consistent left row labels, lean option_list.
    if any_peer_rows >= max(2, int(0.6 * n)) and not left_labels and (left_long_ratio >= 0.25 or right_long_ratio >= 0.25):
        return "option_list"
...
def _group_question_label(lines, var_codes, group_info, left_margin, header_y_cap, med_sz_all, med_sz_black):
    ...
    x_cap = min(cx + 140.0, left_margin + 760.0)
    up = _gather_block_up(lines, best_idx, x_cap=x_cap, max_gap=14.5, x_drift=120.0)
    down = _gather_block_down(
        lines,
        best_idx,
        x_ref=float(getattr(lines[best_idx], "x0", 0.0) or 0.0),
...
def _compute_local_vertical_members(var_codes, vi_seed, x_bucket=10.0, y_tol_gap=(7.0, 52.0), y_span_cap=420.0):
    """
    For a single code that looks like an option anchor (even if it doesn't form a long run),
    gather nearby codes in the same x bucket and similar y-gap cadence to build a small group.
    """
    if not var_codes:
        return [vi_seed]
    seed_line = var_codes[vi_seed]["line"]
    sx = float(getattr(seed_line, "x0", 0.0) or 0.0)
    sy = float(getattr(seed_line, "y0", 0.0) or 0.0)
    key = int(round(sx / x_bucket))

    candidates = []
    for vi, vc in enumerate(var_codes):
        l = vc["line"]
        x = float(getattr(l, "x0", 0.0) or 0.0)
        if int(round(x / x_bucket)) != key:
            continue
        y = float(getattr(l, "y0", 0.0) or 0.0)
        if abs(y - sy) > y_span_cap:
            continue
        candidates.append((vi, y))

    candidates.sort(key=lambda t: t[1])

    pos = None
    for k, (vi, _) in enumerate(candidates):
        if vi == vi_seed:
            pos = k
            break
    if pos is None:
    ...
    # Expand to a contiguous run around the seed with plausible y gaps.
    lo = pos
    while lo - 1 >= 0:
        y_cur = candidates[lo][1]
        y_prev = candidates[lo - 1][1]
        dy = (y_cur - y_prev)
        if y_tol_gap[0] <= dy <= y_tol_gap[1]:
            lo -= 1
        else:
            break

    hi = pos
    while hi + 1 < len(candidates):
        y_cur = candidates[hi][1]
        y_next = candidates[hi + 1][1]
        dy = (y_next - y_cur)
        if y_tol_gap[0] <= dy <= y_tol_gap[1]:
            hi += 1
        else:
    ...
    members = [vi for (vi, _) in candidates[lo : hi + 1]]
    if vi_seed not in members:
        members.append(vi_seed)
...
def extract(pages):
    ...
    for page_idx0, lines in pages:
        ...
        # Fallback: some pages only show [TYPE: ...] markers.
        using_type_markers = False
        if not var_codes:
            type_markers = _collect_type_markers(lines)
            if type_markers:
                var_codes = type_markers
                using_type_markers = True
            else:
                continue
        ...
        min_code_y = min(float(getattr(vc["line"], "y0", 0.0) or 0.0) for vc in var_codes)
        header_y_cap = max(42.0, min(110.0, min_code_y - 60.0))
        ...
        # Group codes by horizontal rows
        ...
        emitted_local_option_keys = set()
        emitted_page_fields = set()

        def emit_field(label, y_ref):
            lab = _norm_text(label)
            if not _label_basic_sane(lab):
                return
            # Guard against obvious single-token short options (still conservative).
            if _label_is_too_optionish(lab):
                return
            key = (lab, int(round(float(y_ref) / 28.0)))
            if key in emitted_page_fields:
                return
            emitted_page_fields.add(key)
            out.append({"form_name": form_name, "field_name": lab, "page": int(page_idx0) + 1})
        ...
        for vi, vc in enumerate(var_codes):
            ...
            # Horizontal option-set collapse
            ...
            if hgid is not None:
                ...
                # Prefer inline-left label; do NOT treat short labels as optionish here (tables often use short row labels).
                ...
                emit_field(label, y_ref=float(getattr(ll, "y0", 0.0) or 0.0))
                continue
            ...
            if vgid is not None and group_kind_v.get(vgid) == "option_list":
                ...
                top_line = var_codes[min(groups_v[vgid]["members"], key=lambda v: float(getattr(var_codes[v]["line"], "y0", 0.0) or 0.0))]["line"]
                emit_field(label, y_ref=float(getattr(top_line, "y0", 0.0) or 0.0))
                continue

            # Singleton/short-run option anchor detection:
            # If there's long anchor text on either side of the code row, don't emit that anchor as a "field".
            peers = _same_row_black_texts(lines, code_line)
            if peers and (_row_has_long_anchor_text(peers, side_wanted=-1) or _row_has_long_anchor_text(peers, side_wanted=1)):
                local_members = _compute_local_vertical_members(var_codes, vi_seed=vi)
                # Only treat as option group if multiple or if the anchor evidence is strong.
                if len(local_members) >= 2 or _row_has_long_anchor_text(peers, side_wanted=-1) or _row_has_long_anchor_text(peers, side_wanted=1):
                    xs = [float(getattr(var_codes[m]["line"], "x0", 0.0) or 0.0) for m in local_members]
                    ys = [float(getattr(var_codes[m]["line"], "y0", 0.0) or 0.0) for m in local_members]
                    local_key = (int(round(_percentile(xs, 0.5) / 10.0)), int(round(min(ys) / 22.0)))
                    if local_key not in emitted_local_option_keys:
                        emitted_local_option_keys.add(local_key)
                        label = _group_question_label(
                            lines,
                            var_codes,
                        ...
                        label = _norm_text(label)
                        if _label_basic_sane(label):
                            emit_field(label, y_ref=min(ys))
                    continue

            # Non-option: per-code labeling (including row-table groups)
            # Inline-left is the best at capturing short row labels (HEENT/Thorax/Abdomen/Evaluator/etc).
            ...
            # When we're operating on [TYPE: ...] fallback pages, avoid emitting the marker itself.
            if using_type_markers and _RE_TYPE_PREFIX.match(label):
                continue

            emit_field(label, y_ref=cy)
