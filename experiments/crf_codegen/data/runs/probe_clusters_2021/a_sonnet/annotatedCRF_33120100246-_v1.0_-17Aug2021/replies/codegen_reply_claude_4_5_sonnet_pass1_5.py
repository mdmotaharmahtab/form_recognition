Looking at the quality gate failures, I need to:

1. **Fix cluster 10 coverage** (pages 4, 6): Dense blue link pages that are being skipped - these ARE field enumerations, not TOC
2. **Fix missing table headers** (pages 46, 50, 302): Chemistry/Urinalysis fields in table layouts
3. **Fix page 71**: C-SSRS questions being skipped
4. **Fix page 83**: "Met/Not Met" field missing
5. **Filter out non-fields**: Headers like "Suicidal Ideation", "Intensity of Ideation", table column headers
6. **Fix form attribution**: Wrong form_name on page 302

The key issue is my blue link filtering is too aggressive - I'm skipping cluster 10/11 pages that have 30+ blue links, but these are legitimate field enumerations, not TOC pages.
