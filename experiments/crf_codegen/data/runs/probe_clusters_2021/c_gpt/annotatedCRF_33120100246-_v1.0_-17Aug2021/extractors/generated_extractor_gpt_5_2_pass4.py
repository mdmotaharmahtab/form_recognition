# Observed layouts: (1) TOC/list pages with many blue numbered lines and no red [CODE]/[TYPE] markers,
# (2) form pages where each data-entry field is annotated by red bracketed codes and/or red [TYPE: ...] lines,
# with human labels in a left text column and answer options/values in right columns.
# Strategy: detect/carry forward a prominent top title as form_name; detect fields via red bracketed codes,
# recover field_name by joining nearby left-column label lines (preferring bold question lines), excluding Row/option text.
...
def _find_label_block(lines, y_anchor, mid_x, median_size):
...
def _is_read_only_near(lines, y_anchor) -> bool:
    # Template marker; safe literal usage as a repeated system annotation.
    for ln in lines:
        if not ln.non_black:
            continue
        t = (ln.text or "").strip()
        if "Read-only field" in t and (y_anchor - 120) <= ln.y0 <= (y_anchor + 40):
            return True
    return False
...
def _extract_fields_from_page(lines, page_1based, form_name):
    ...
    for g in groups:
        ...
        if _is_read_only_near(lines, y):
            continue

        block = _find_label_block(lines, y, mid, med_size)
        if not block:
            continue
