...
def _detect_form_title_and_size(lines) -> Tuple[str, float]:
    ...
    title_line = cand[0]
    return _norm(title_line.text), float(getattr(title_line, "size", 0.0) or 0.0)
...
def _detect_table_headers(lines, title_size: float) -> List[str]:
    ...
    black_sizes = [
        float(l.size)
        for l in lines
        if (not getattr(l, "non_black", False)) and float(getattr(l, "y0", 0.0)) > 120.0
    ]
    body_med = _median(black_sizes) or 7.0

    band = []
    for i, l in enumerate(lines):
        y = float(getattr(l, "y0", 0.0))
        if y < 135.0 or y > 220.0:
            continue
        if getattr(l, "non_black", False):
            continue
        if title_size and float(getattr(l, "size", 0.0)) >= title_size * 0.95:
            continue
        t = _norm(l.text)
