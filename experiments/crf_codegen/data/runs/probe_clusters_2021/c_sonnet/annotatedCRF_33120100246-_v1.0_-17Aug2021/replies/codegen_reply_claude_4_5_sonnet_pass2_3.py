...
1. **Page 84**: Extracting instructional text and multi-sentence criterion descriptions (not data-entry fields)
2. **Page 202**: Extracting section headers like "Suicidal behavior:" (not fields)

The program needs better structural filters to distinguish:
- Data-entry field labels (short, specific prompts)
- Instructional text (imperative, conditional)
- Criterion descriptions (long, multi-sentence)
- Section headers (short labels ending with colon, no continuation)
    ...
    for page_idx, lines in pages:
        ...
        while i < len(lines):
            ...
            if not ln.non_black and ln.size >= 7.5 and ln.x0 < 150:
                ...
                # Skip standalone section headers: short text ending with colon, no continuation
                if len(full_label) < 60 and full_label.rstrip().endswith(':') and j == i + 1:
                    i = j
                    continue
                ...
                # Skip criterion-style text: starts with backslash-digit pattern
                ...
                # Detect by: multiple commas + length, or semicolons in mid-text
