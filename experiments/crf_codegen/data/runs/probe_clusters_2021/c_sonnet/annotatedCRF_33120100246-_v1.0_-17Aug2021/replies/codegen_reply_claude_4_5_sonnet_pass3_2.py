...
1. **Missing fields on pages like 288, 460** (cluster 2, 45% covered): These show a clear tabular layout with field names like "Skin and Mucosae", "Neurological", "Extremities" in dark gray (#454545) at specific x-positions (around x=46.5).

2. **False positives**: Extracting instructional text, answer options, and non-field labels as fields.

3. **Remove hardcoded blocklists**: Use structural discriminators instead.

Let me analyze the patterns:

- **Real fields in the new layout**: Dark gray text (#454545) at x ≈ 46.5, size 7.8, followed by Yes/No options
- **False positives**: Long instructional sentences, answer option descriptions, continuation text that's not actually part of field labels
...
def extract_fields_from_page(lines):
    ...
    # Pattern: dark gray (#454545) text at left margin, followed by Yes/No options
...
def extract_tabular_fields(lines, seen):
    ...
    # Find lines that are dark gray (#454545), left-aligned, size ~7.8
    for i, line in enumerate(lines):
        ...
        # Dark gray left-margin labels (tabular form layout)
        if (line.color == '#454545' and 
...
def extract_question_fields(lines, seen):
    ...
    while i < len(lines):
        ...
        if is_machine_code(line.text) or (line.non_black and line.color == '#ff0000'):
            i += 1
            continue
        ...
        # Skip dark gray tabular labels (handled separately)
        if line.color == '#454545':
            i += 1
            continue
        ...
        if is_field_label(line, text, lines, i):
            ...
            while j < len(lines):
                ...
                # Stop at machine code or structural break
                if is_machine_code(next_line.text) or (next_line.non_black and next_line.color == '#ff0000'):
                    break
                
                # Stop at dark gray labels
                if next_line.color == '#454545':
                    break
...
def is_field_label(line, text, lines, idx):
    ...
    # Must be black text (not colored)
    if line.non_black:
        return False
