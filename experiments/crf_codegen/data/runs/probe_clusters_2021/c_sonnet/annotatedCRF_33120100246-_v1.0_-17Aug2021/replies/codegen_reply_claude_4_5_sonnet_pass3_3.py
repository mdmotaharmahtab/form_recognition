Looking at the error, the issue is that `Line` objects don't have a `color` attribute. I need to check what attributes are actually available and adjust the program accordingly.

Let me rewrite the program to fix the crash and handle the line attributes correctly:
...
The key fix: I removed all references to `line.color` (which doesn't exist) and instead rely on `line.non_black` (which does exist) to identify non-black text. The tabular field extraction now uses only `line.non_black` along with size and position to identify dark gray labels, without needing the specific color value.
