...
# --- Regexes ---
_RE_BRACKET_TOKEN = re.compile(r"\[[A-Za-z0-9]{2,}\]")
_RE_FIELD_CODE_EXACT = re.compile(r"^\[[A-Za-z0-9]{2,}\]$")
...
def _extract_code_tokens_from_text(t: str) -> List[str]:
    ...
    for tok in tokens:
        if _RE_TYPELIKE.match(tok):
            continue
        inner = tok[1:-1]
        if ":" in inner:  # avoid "[SOMETHING:...]" blobs
            continue
        out.append(tok)
    return out
...
def _looks_like_short_option(line, page_right: float) -> bool:
    ...
    # strong signal: short + far-right + colored + small
    if x0 >= page_right * 0.58 and non_black and size <= 11.0 and len(t) <= 18:
        return True

    # weaker signal: very short + far-right + small (even if black)
    if x0 >= page_right * 0.68 and size <= 10.5 and not bold and len(t) <= 10:
        return True
...
def _is_label_candidate(line, page_right: float) -> bool:
    ...
    if t.startswith("[") and t.endswith("]") and len(_extract_code_tokens_from_text(t)) > 0:
        ...
        # (still allow if there is meaningful non-bracket text; handled elsewhere)
...
def _extract_label_near_code(
    ...
    left_col_max, right_col_min = _column_thresholds(page_right)
    ...
    if inline and not _is_typelike_line(inline):
        # Ensure it's not mostly bracket junk
    ...
    def column_accept(ln) -> bool:
        ...
        # If code is right-ish, labels typically live in left column, but sometimes align near left band.
        return x0 <= left_col_max + 10.0
    ...
    if best_anchor is None:
        max_fwd = max(150.0, min(260.0, page_right * 0.45))
    ...
    for j in range(best_anchor - 1, span_start - 1, -1):
        ...
        y0 = float(_g(ln, "y0", 0.0) or 0.0)
        gap = prev_y - y0
        # allow slightly larger gaps for looser table leading
        if gap > 20.0:
            break
        start = j
        prev_y = y0
    ...
    for j in range(start, min(stop_excl, span_end)):
        ...
        if prev_included_y is not None:
            ...
            # allow bigger gaps than before to avoid truncating long criteria-like labels
            if gap > 26.0:
                # don't consume long definitions; stop once we have at least something
                if parts:
                    break
        parts.append(_g(ln, "text", "") or "")
        prev_included_y = y0
