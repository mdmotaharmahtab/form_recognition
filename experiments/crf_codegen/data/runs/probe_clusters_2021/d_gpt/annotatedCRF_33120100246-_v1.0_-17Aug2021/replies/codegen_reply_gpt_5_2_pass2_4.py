...
def _extract_code_tokens_from_line(line, page_right: float) -> List[str]:
    ...
    or when token(s) are clearly appended to a compact label (often end-of-line).
    """
    raw = _g(line, "text", "") or ""
    t = _clean_text(raw)
    ...
    # If it's a long paragraph in left column, reject (common OCR brackets for abbreviations).
    if x0 <= left_col_max + 10.0 and not bold and not non_black and size >= 10.5 and remainder_len >= 120:
        return []
    ...
    # Accept if tokens are appended to a compact label (token near line end).
    endish = bool(re.search(r"\]\s*[\)\.\:\?]*\s*$", raw))
    if endish and remainder_len <= 125:
        # favor label-ish remainders
        if remainder.endswith("?") or remainder.endswith(":") or bold or _is_numbered_start_text(remainder) or remainder_len <= 70:
            return tokens

    # Accept if the line is already a reasonable label candidate (compact, label-ish).
    if remainder_len <= 90 and (bold or remainder.endswith("?") or remainder.endswith(":") or _is_numbered_start_text(remainder)):
        return tokens
...
def _extract_label_near_code(
    ...
    inline_prefix = _extract_inline_label_from_code_line_text(_g(code_ln, "text", "") or "")
    ...
    # Seed anchor preference with inline prefix on the code line (if it's plausible).
    preferred_anchor = neigh_best
    preferred_score = neigh_best_score
    if inline_prefix and not _is_typelike_line(inline_prefix):
        # don't immediately return: we may need wrapped continuation below (especially numbered criteria)
        if not _RE_MOSTLY_PUNCT.match(inline_prefix):
            preferred_anchor = code_idx
            preferred_score = max(preferred_score, 4 + (4 if _is_numbered_start_text(inline_prefix) else 0))
    ...
    for j in range(code_idx - 1, span_start - 1, -1):
        ...
        if not column_accept(ln):
            continue
        sc = _labelish_score(ln)
        # strong distance weighting prevents latching onto headers/definitions when a question is nearby
    ...
    if inline_prefix and best_anchor == code_idx:
        anchor_text = inline_prefix
    ...
    # Collect downward. If anchor is the code line itself (inline label+code),
    # allow continuation below the code line (common for wrapped questions/criteria).
    stop_excl = span_end if (best_anchor == code_idx and inline_prefix) else (code_idx if start <= code_idx else span_end)
    ...
    for j in range(start, min(stop_excl, span_end)):
        ...
        # prefer the inline prefix to avoid trailing bracket tokens.
        if j == code_idx and inline_prefix:
            t = inline_prefix
