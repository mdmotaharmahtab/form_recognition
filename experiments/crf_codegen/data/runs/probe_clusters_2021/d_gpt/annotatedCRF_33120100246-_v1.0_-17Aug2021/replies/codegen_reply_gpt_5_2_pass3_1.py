```python
# Observed layouts: (1) TOC/index pages with many blue numbered entries (no fields),
# (2) standard annotated CRF pages where red bracketed variable codes sit near entry cells,
# (3) table pages where a left row label + a column header define a fillable cell,
# (4) continuation pages carrying only repeated row blocks (row label + options) and codes.
# Strategy: carry forward the most prominent top title as form_name, use red [VARNAME]
# anchors plus tolerant geometry to recover nearby human labels, joining wrapped labels.
