...
def extract(pages):
    ...
    for page_index0, lines in pages:
        ...
        for code_ln in var_codes:
            if _is_read_only_near_code(lines, ys, code_ln):
                continue
...
def _is_read_only_near_code(lines, ys, code_ln):
    lo, hi = _window(lines, ys, code_ln.y0 - 50.0, code_ln.y0 + 50.0)
    for i in range(lo, hi):
        ...
        if not t.startswith("["):
            continue
        if "read-only" in t.lower() or "readonly" in t.lower():
            return True
    return False
...
def _nearest_above_near_code(lines, ys, code_ln, page_w, header_bands, y_pad=140.0):
    ...
    for i in range(hi - 1, lo - 1, -1):
        ...
        if _is_option_like_candidate(lines, ys, ln, txt, page_w):
            continue
...
def _promote_option_to_question(lines, ys, option_ln, page_w, header_bands, med_sz):
    ...
    for i in range(hi - 1, lo - 1, -1):
        ...
        wc = _word_count(label)
        is_promptish = label.endswith("?") or label.endswith(":")
...
def _labels_followed_by_code(lines, ys, page_w, med_sz, header_bands):
    ...
    for i, ln in enumerate(lines):
        ...
        wc0 = _word_count(txt0)
        if wc0 >= 14 and not (ln.bold or txt0.endswith("?") or txt0.endswith(":")):
            continue

        if _is_option_like_candidate(lines, ys, ln, txt0, page_w):
            continue
        ...
        for j in range(lo, hi):
            ...
            if not _is_var_code_line(c):
                continue
            if _is_read_only_near_code(lines, ys, c):
                continue
