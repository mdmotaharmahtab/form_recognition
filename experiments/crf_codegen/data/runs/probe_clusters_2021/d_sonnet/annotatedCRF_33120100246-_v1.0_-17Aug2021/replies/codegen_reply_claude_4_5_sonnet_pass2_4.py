...
1. **Missing "Investigator comment" fields** - These appear right after the dropdown assay selections, at the same x-position but slightly lower y-coordinate
2. **Missing inclusion/exclusion criteria on pages 83-84** - These are long multi-line criteria that my current program filters out as "instructions"
3. **Missing fields on page 427** - These are follow-up fields with "If Yes," prefixes

Let me analyze the sample pages to understand the patterns:

- **Page 240/469**: The "Investigator comment" fields appear at x=47.8, after the dropdown list, with normal size (7.8)
- **Page 298/473**: Same pattern for hematology
- **Page 87/147**: The exclusion criteria items are in table format with "Met/Not Met" options - these should be extracted
- **Page 83-84**: Long inclusion/exclusion criteria that start with `\1.\`, `\2.\` etc.

The key fixes needed:
1. Don't skip "Investigator comment" labels (they're short but valid fields)
2. Handle long multi-line inclusion/exclusion criteria (even though they look like instructions)
3. Better handling of fields with "If Yes," prefixes
    ...
    for page_idx, lines in pages:
        ...
        while i < len(sorted_lines):
            ...
            if len(text) < 15 and (line.bold or line.x0 > 400):
                ...
                if not ('comment' in text.lower() or text.startswith('If Yes,')):
                    i += 1
                    continue
            ...
            elif re.match(r'^\\[0-9]+\\.\\', text) and line.x0 < 300 and line.size >= 7.5:
                is_field_label = True
            ...
            if len(field_label) < 8:
                if not ('comment' in field_label.lower() or field_label.startswith('If Yes,')):
                    i = j
                    continue
