...
1. **Fix page 549 extraction** - fields are present but not extracted
2. **Remove hardcoded literal text filters** - replace with structural discriminators
3. **Handle cluster 1 pages** (0% coverage) - these appear to be table of contents/schedule pages

Let me analyze the sample from page 14 (cluster 1): it shows a two-column layout with numbers on the left and form titles on the right in blue. These are NOT data-entry fields - they're TOC entries, so 0% coverage is correct.

The main issue is **page 549** missing fields and the **hardcoded text filters** that need to be replaced with structural logic.
