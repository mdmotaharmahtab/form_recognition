...
# repo layout: scripts/ (this file + modules) and dataiku_notebook_pipeline/
# (notebook + folder_code staging + downloaded runs) are siblings
PIPELINE_DIR = os.path.join(os.path.dirname(HERE), "dataiku_notebook_pipeline")
...
Per document (under `runs/<RUN_ID>/<doc_key>/`):

| artifact | meaning |
|---|---|
| `clusters.json` | stage-0 layout clusters, representative pages, selected `theta`, `status`, `elapsed_s` |
...
| `codegen_prompt_pass<N>.txt`, `*_<tag>_pass<N>.*` | multi-pass specialists only (documents whose layout families exceed one prompt): per-pass prompts/replies/trails/CSVs; the plain-tag CSV/trail/call-log above are the MERGED document-level outputs, and `codegen_prompt.txt` is not written for such documents |
| `oid_mapping_<tag>.csv` | optional name->OID funnel result: status / oid / via / reason (mapping cell) |
| `timings_<tag>.json` | time profile: `stage0_s`, LLM time by call kind, sandbox time, `export_s`, `total_s` |
| `llm_calls_<tag>.jsonl` | every LLM call verbatim (prompt + reply + seconds + sizes) |
...
`experiments/recipe_prototype/scripts/`). It tokenizes the `llm_calls` records
