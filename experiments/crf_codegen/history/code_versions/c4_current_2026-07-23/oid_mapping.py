...
class MapResult:
    ...
    # why a pair is NOT mapped (empty for mapped pairs) - feeds the run error
    # report so unmapped volume can be attributed to a funnel layer:
    #   empty_input | no_form_scope | no_field_candidates | lexical_tie |
    #   ranker_refused
    reason: str = ""
...
def map_field(form: str, field: str, lib: list[dict]) -> MapResult:
    ...
    if not form.strip() or not field.strip():
        return MapResult(form, field, UNMAPPED, reason="empty_input")
    scoped = scope_form(form, lib)
    if not scoped:
        return MapResult(form, field, UNMAPPED, reason="no_form_scope")
    ...
    for e in scoped:
        ...
        if key not in per_oid or s > per_oid[key][0]:
            per_oid[key] = (s, e)
    if not per_oid:
        return MapResult(form, field, UNMAPPED, reason="no_field_candidates")
...
def rank_cases(cases: list[MapResult], llm) -> None:
    ...
    for c, oid in all_picks:
        if oid:
            ...
            c.status, c.oid, c.reason = MAPPED, oid, ""
        else:
            c.status, c.oid, c.reason = UNMAPPED, None, "ranker_refused"
