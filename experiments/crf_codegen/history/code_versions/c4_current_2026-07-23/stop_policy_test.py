...
import run_report
from common import Line, pick_title_context_pages
...
def main() -> None:
    ...
    with open(os.path.join(tmp, "clusters.json"), "w", encoding="utf-8") as f:
    ...
    with open(os.path.join(mp, "clusters.json"), "w", encoding="utf-8") as f:
    ...
    for stale in ("fields_codegen_tag_pass2.csv", "fields_codegen_tag_pass9.csv"):
        with open(os.path.join(mp, stale), "w", encoding="utf-8", newline="") as f:
            f.write("form_name,field_name,page\nSTALE,STALE,1\n")
    with open(os.path.join(mp, "codegen_prompt.txt"), "w", encoding="utf-8") as f:
    ...
    try:
        ...
        for name, fn in ORIGINALS.items():
            setattr(rci, name, fn)
    with open(os.path.join(mp, "fields_codegen_tag.csv"), encoding="utf-8", newline="") as f:
    ...
    check("multipass purges stale artifacts",
          not os.path.exists(os.path.join(mp, "fields_codegen_tag_pass9.csv"))
          and not os.path.exists(os.path.join(mp, "codegen_prompt.txt")))
    ...
    with open(os.path.join(mp, "codegen_trail_tag.json"), encoding="utf-8") as f:
        trail23 = json.load(f)
...
"""
    ...
    fam_meta = {"pages": 40,
                ...
                "clusters": [
                    ...
                    {"n_pages": 5, "pages": list(range(35, 40)),
                     "representatives": [36]}]}
    with open(os.path.join(fam_dir, "clusters.json"), "w", encoding="utf-8") as f:
        json.dump(fam_meta, f)
    for p in (3, 5, 8, 20, 28, 31, 37):
        stem = "title" if p in (8, 28) else "rep"
        with open(os.path.join(fam_dir, f"{stem}_p{p}.txt"), "w", encoding="utf-8") as f:
    ...
    for fn in stale + keep:
        with open(os.path.join(purge_dir, fn), "w", encoding="utf-8") as f:
            f.write("x")
    rci.purge_tag_artifacts(purge_dir, "tag")
    check("same-tag artifact purge prevents stale export reuse",
          not any(os.path.exists(os.path.join(purge_dir, fn)) for fn in stale)
          and all(os.path.exists(os.path.join(purge_dir, fn)) for fn in keep))
    locked = os.path.join(purge_dir, "fields_codegen_tag.csv")
