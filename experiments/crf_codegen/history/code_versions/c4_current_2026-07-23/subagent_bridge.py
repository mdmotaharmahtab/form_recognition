...
def cmd_finalize_code(dockey: str, reply_path: str) -> None:
    ...
    with open(os.path.join(outdir, "generated_extractor.py"), "w", encoding="utf-8") as f:
        f.write(source + "\n")
    full = run_extractor(source, pdf)
    with open(os.path.join(outdir, "fields_codegen.csv"), "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
