...
CLI_OUT = OUT_DIR                                 # default corpus run root
AUDIT = os.path.join(PROJECT_ROOT, "eval_assets")  # truth/ + packets/
...
def cmd_sample(_args: argparse.Namespace) -> None:
    ...
    for fname in sorted(os.listdir(CORPUS)):
        if not fname.lower().endswith(".pdf"):
            continue
        key = doc_key(fname)
        cl_path = art(os.path.join(CLI_OUT, key), "clusters.json")
...
def load_export(run: dict, doc_key_: str) -> list[dict] | None:
    ...
    p = art(d, f"fields_codegen_{run['tag']}.csv")
...
def cmd_report(_args: argparse.Namespace) -> None:
    ...
    out = os.path.join(PROTO, "notebooks", "accuracy_report.html")
