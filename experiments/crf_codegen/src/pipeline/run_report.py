...
_WARNING_CODES = [
    ...
    ("extracted on >=70% of ALL pages", "warn_furniture_candidates"),
    ("layout scope produced ZERO records", "warn_scope_zero_records"),
    ("layout scope produced only", "warn_scope_low_records"),
    ("hardcoded literal text strings", "warn_text_blocklists"),
]
...
def collect_run_events(out_dir: str, tag: str, summary: list[dict]) -> list[dict]:
    ...
    for row in summary:
        doc = row.get("doc", "")
        trail_obj = {}
        trail_path = art(os.path.join(out_dir, doc), f"codegen_trail_{tag}.json")
