...
def induce_document(call, model: str, tag: str, pdf: str, outdir: str,
    ...
    calls_path = os.path.join(outdir, f"llm_calls_{tag}.jsonl")
    try:
...
def write_timings(outdir: str, tag: str, profile: dict | None,
    ...
    try:
        with open(os.path.join(outdir, f"timings_{tag}.json"), "w", encoding="utf-8") as f:
...
def purge_tag_artifacts(outdir: str, tag: str) -> None:
    ...
    for fn in os.listdir(outdir):
        if fn in exact or fn.startswith(prefixes):
            ...
            # locked/permission-denied CSV from an older run masquerade as the
            # current result if generation or export later fails.
...
def run_document(call, model: str, tag: str, key: str, pdf: str, outdir: str,
    ...
    for fn in os.listdir(outdir):
        if fn.startswith(stale_prefixes) or fn.startswith("codegen_prompt_pass") \
                or fn in ("codegen_prompt.txt", f"generated_extractor_{tag}.py"):
            try:
                os.remove(os.path.join(outdir, fn))
            except OSError:
                pass
    ...
    for gi, g in enumerate(groups, 1):
        ...
        with open(os.path.join(outdir, f"codegen_prompt_pass{gi}.txt"), "w",
                  encoding="utf-8") as f:
            f.write(prompt)
        ...
        csv_path = os.path.join(outdir, f"fields_codegen_{ptag}.csv")
    ...
    with open(os.path.join(outdir, f"fields_codegen_{tag}.csv"), "w",
    ...
    with open(os.path.join(outdir, f"llm_calls_{tag}.jsonl"), "w", encoding="utf-8") as out:
        for gi in range(1, len(groups) + 1):
            p = os.path.join(outdir, f"llm_calls_{tag}_pass{gi}.jsonl")
            if os.path.isfile(p):
