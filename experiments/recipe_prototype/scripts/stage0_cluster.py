...
def run(path: str) -> dict:
    ...
    for pattern in ("rep_*", "title_*"):
        for stale in glob.glob(os.path.join(out, pattern)):
            os.remove(stale)
    doc = fitz.open(path)
    try:  # close in finally: a mid-run exception must not leak the handle
        ...
        for p in title_pages:
            dump_rep_page(page_lines[p], os.path.join(out, f"title_p{p + 1}.txt"))
            doc[p].get_pixmap(dpi=100).save(
                os.path.join(out, f"title_p{p + 1}.png"))
